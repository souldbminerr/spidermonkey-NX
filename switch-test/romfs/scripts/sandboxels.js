(function initShim() {
  ...
  function parseColor(c) {
    ...
    if (s[0] === "#") {
      ...
      const m = s.match(/[\d.]+/g) || [];
      out = [+m[0] | 0, +m[1] | 0, +m[2] | 0, m[3] !== undefined ? Math.round(+m[3] * 255) : 255];
    } else if (s.startsWith("hsl")) {
      const m = s.match(/[\d.]+/g) || [];
      const rgb = hslToRgb(+m[0], +m[1], +m[2]);
      out = [rgb[0], rgb[1], rgb[2], m[3] !== undefined ? Math.round(+m[3] * 255) : 255];
    } else if (NAMED[s]) {
      out = NAMED[s].slice();
    }
    _colorCache[c] = out;
  ...
  G.__sbxParseColor = parseColor;

  // ---- 2D context ---------------------------------------------------------
  // One implementation backs both the primary (framebuffer) and offscreen
  // (CPU buffer) canvases. `canvas._primary` selects the path.
  function Ctx(canvas) {
    this.canvas = canvas;
  ...
  Ctx.prototype._alpha = function () {
    let a = this.globalAlpha;
    if (a < 0) a = 0; else if (a > 1) a = 1;
    return a;
  };
  // Write an alpha-blended rect into an offscreen Uint8 RGBA buffer.
  function bufFill(canvas, x, y, w, h, r, g, b, a) {
    const W = canvas.width, H = canvas.height, buf = canvas._buf;
    let x0 = Math.max(0, x | 0), y0 = Math.max(0, y | 0);
    let x1 = Math.min(W, (x + w) | 0), y1 = Math.min(H, (y + h) | 0);
    for (let py = y0; py < y1; py++) {
      let o = (py * W + x0) * 4;
      for (let px = x0; px < x1; px++, o += 4) {
        if (a >= 255) { buf[o] = r; buf[o + 1] = g; buf[o + 2] = b; buf[o + 3] = 255; }
        else if (a > 0) {
          const ia = 255 - a;
          buf[o] = (r * a + buf[o] * ia) / 255;
          buf[o + 1] = (g * a + buf[o + 1] * ia) / 255;
          buf[o + 2] = (b * a + buf[o + 2] * ia) / 255;
  ...
  Ctx.prototype.fillRect = function (x, y, w, h) {
    ...
    const a = Math.round(c[3] * this._alpha());
    if (this.canvas._primary) nx.gfx.fillRect(x, y, w, h, c[0], c[1], c[2], a);
    else bufFill(this.canvas, x, y, w, h, c[0], c[1], c[2], a);
  };
  Ctx.prototype.clearRect = function (x, y, w, h) {
    if (this.canvas._primary) {
      // Primary clearRect == reveal page background. Bootstrap keeps the
      // current bg color in G.__sbxBg.
      const bg = G.__sbxBg || [0, 0, 0];
      nx.gfx.fillRect(x, y, w, h, bg[0], bg[1], bg[2], 255);
    } else {
      const W = this.canvas.width, buf = this.canvas._buf;
      let x0 = Math.max(0, x | 0), y0 = Math.max(0, y | 0);
      let x1 = Math.min(W, (x + w) | 0), y1 = Math.min(this.canvas.height, (y + h) | 0);
      for (let py = y0; py < y1; py++) {
        let o = (py * W + x0) * 4;
        for (let px = x0; px < x1; px++, o += 4) { buf[o] = buf[o + 1] = buf[o + 2] = buf[o + 3] = 0; }
      }
    }
  };
  Ctx.prototype.strokeRect = function (x, y, w, h) {
    ...
    const draw = (px, py, pw, ph) => {
      if (this.canvas._primary) nx.gfx.fillRect(px, py, pw, ph, c[0], c[1], c[2], a);
      else bufFill(this.canvas, px, py, pw, ph, c[0], c[1], c[2], a);
    };
    draw(x, y, w, lw); draw(x, y + h - lw, w, lw);
    draw(x, y, lw, h); draw(x + w - lw, y, lw, h);
  };
  // Path ops: we approximate strokes as 1px-ish lines via small rects. Only
  // the cursor uses these, so cheap axis-aligned segments are good enough.
  Ctx.prototype.beginPath = function () { this._path = []; };
  Ctx.prototype.moveTo = function (x, y) { this._path = [[x, y]]; };
  Ctx.prototype.lineTo = function (x, y) { this._path.push([x, y]); };
  Ctx.prototype.closePath = function () {};
  Ctx.prototype.arc = function (x, y, rad) { this._path.push([x, y]); };
  Ctx.prototype.rect = function (x, y, w, h) { this._lastRect = [x, y, w, h]; };
  Ctx.prototype.stroke = function () {
    ...
    for (let i = 1; i < this._path.length; i++) {
      const [x0, y0] = this._path[i - 1], [x1, y1] = this._path[i];
      const dx = x1 - x0, dy = y1 - y0;
      const steps = Math.max(1, Math.ceil(Math.max(Math.abs(dx), Math.abs(dy))));
      for (let s = 0; s <= steps; s++) {
        put((x0 + dx * s / steps) | 0, (y0 + dy * s / steps) | 0, lw, lw);
      }
    }
  };
  Ctx.prototype.fill = function () {};
  Ctx.prototype._fontScale = function () {
    const m = /([0-9.]+)px/.exec(this.font || "");
    const px = m ? parseFloat(m[1]) : 16;
    return Math.max(1, Math.round(px / 8));
  };
  // 8x8 bitmap text via the native font (primary/framebuffer only).
  Ctx.prototype.fillText = function (text, x, y) {
    if (!this.canvas._primary) return;
    const c = parseColor(this.fillStyle);
    const s = this._fontScale();
    // canvas text baseline is the bottom; our font draws from the top.
    nx.gfx.text("" + text, x | 0, (y - 8 * s) | 0, c[0], c[1], c[2], Math.round(c[3] * this._alpha()), s);
  };
  ...
  Ctx.prototype.measureText = function (t) { return { width: ("" + t).length * 8 * this._fontScale() }; };
  ...
  Ctx.prototype.getImageData = function (x, y, w, h) {
    ...
    if (!this.canvas._primary && this.canvas._buf) {
      const W = this.canvas.width, buf = this.canvas._buf;
      for (let py = 0; py < h; py++) {
        ...
        for (let px = 0; px < w; px++) {
          ...
          const so = (sy * W + sx) * 4, doo = (py * w + px) * 4;
          out[doo] = buf[so]; out[doo + 1] = buf[so + 1];
          out[doo + 2] = buf[so + 2]; out[doo + 3] = buf[so + 3];
        }
      }
    }
    return { data: out, width: w, height: h };
  };
  Ctx.prototype.putImageData = function (img, dx, dy) {
    if (this.canvas._primary || !this.canvas._buf) return;
    const W = this.canvas.width, buf = this.canvas._buf;
    for (let py = 0; py < img.height; py++) {
      const ty = dy + py; if (ty < 0 || ty >= this.canvas.height) continue;
      for (let px = 0; px < img.width; px++) {
        const tx = dx + px; if (tx < 0 || tx >= W) continue;
        const so = (py * img.width + px) * 4, to = (ty * W + tx) * 4;
        buf[to] = img.data[so]; buf[to + 1] = img.data[so + 1];
        buf[to + 2] = img.data[so + 2]; buf[to + 3] = img.data[so + 3];
      }
    }
  };
  ...
  // drawImage(src, dx, dy[, dw, dh])  and the 9-arg sub-rect form.
  // src is one of our canvases (has _buf) or a stub Image (ignored).
  Ctx.prototype.drawImage = function (src) {
    if (!src || !src._buf) return;            // Image/texture: not loaded -> skip
    const a = Math.round(255 * this._alpha());
    let dx = 0, dy = 0;
    if (arguments.length === 3 || arguments.length === 5) { dx = arguments[1] | 0; dy = arguments[2] | 0; }
    else if (arguments.length === 9) { dx = arguments[5] | 0; dy = arguments[6] | 0; }
    if (this.canvas._primary) {
      // Composite an offscreen layer onto the framebuffer (native, fast).
      nx.gfx.blit(src._buf, src.width, src.height, dx, dy, 1, 1, a);
    } else {
      // Offscreen <- offscreen copy (rare: scratch/scaling canvases).
      const sb = src._buf, db = this.canvas._buf, SW = src.width, DW = this.canvas.width;
      for (let py = 0; py < src.height; py++) {
        const ty = dy + py; if (ty < 0 || ty >= this.canvas.height) continue;
        for (let px = 0; px < SW; px++) {
          const tx = dx + px; if (tx < 0 || tx >= DW) continue;
          const so = (py * SW + px) * 4, to = (ty * DW + tx) * 4;
          if (sb[so + 3] === 0) continue;
          db[to] = sb[so]; db[to + 1] = sb[so + 1]; db[to + 2] = sb[so + 2]; db[to + 3] = sb[so + 3];
        }
      }
    }
  };
  ...
  function HTMLCanvas(primary) {
    ...
    this._listeners = {};
    if (!primary) this._setSize(1, 1);
  }
  HTMLCanvas.prototype._setSize = function (w, h) {
    w = Math.max(1, w | 0); h = Math.max(1, h | 0);
    if (w === this._w && h === this._h) return;
    this._w = w; this._h = h;
    if (!this._primary) this._buf = new Uint8Array(w * h * 4); // zero = transparent
  };
  Object.defineProperty(HTMLCanvas.prototype, "width", {
    get() { return this._w; }, set(v) { this._setSize(v, this._h); },
  });
  Object.defineProperty(HTMLCanvas.prototype, "height", {
    get() { return this._h; }, set(v) { this._setSize(this._w, v); },
  });
  HTMLCanvas.prototype.getContext = function () {
    if (!this._ctx) this._ctx = new Ctx(this);
    return this._ctx;
  };
  HTMLCanvas.prototype.getBoundingClientRect = function () {
    return { left: 0, top: 0, right: this._w, bottom: this._h, width: this._w, height: this._h, x: 0, y: 0 };
  };
  HTMLCanvas.prototype.toDataURL = function () { return "data:,"; };
  HTMLCanvas.prototype.addEventListener = function (t, fn) { (this._listeners[t] ||= []).push(fn); };
  HTMLCanvas.prototype.removeEventListener = function () {};
  HTMLCanvas.prototype.setAttribute = function () {};
  HTMLCanvas.prototype.getAttribute = function () { return null; };
  HTMLCanvas.prototype.appendChild = function (c) { return c; };
  Object.defineProperty(HTMLCanvas.prototype, "clientWidth", { get() { return this._w; } });
  Object.defineProperty(HTMLCanvas.prototype, "clientHeight", { get() { return this._h; } });
  Object.defineProperty(HTMLCanvas.prototype, "offsetWidth", { get() { return this._w; } });
  Object.defineProperty(HTMLCanvas.prototype, "offsetHeight", { get() { return this._h; } });
  G.__sbxHTMLCanvas = HTMLCanvas;

  // ---- generic style / classList / element stub ---------------------------
  function makeStyle() {
    // Swallow arbitrary property reads/writes (the engine sets dozens).
    return new Proxy({}, {
  ...
  StubElement.prototype.appendChild = function (c) { this.children.push(c); this.childNodes.push(c); if (c) c.parentNode = this; return c; };
  StubElement.prototype.removeChild = function (c) {
    const i = this.children.indexOf(c); if (i >= 0) this.children.splice(i, 1); return c;
  };
  StubElement.prototype.insertBefore = function (c) { this.children.push(c); return c; };
  StubElement.prototype.append = function (...cs) { cs.forEach(c => this.children.push(c)); };
  StubElement.prototype.prepend = function () {};
  StubElement.prototype.remove = function () {};
  StubElement.prototype.cloneNode = function () { return new StubElement(this.tagName); };
  StubElement.prototype.setAttribute = function (k, v) { this._attrs[k] = v; if (k === "element") this.element = v; };
  StubElement.prototype.getAttribute = function (k) { return k in this._attrs ? this._attrs[k] : null; };
  StubElement.prototype.removeAttribute = function (k) { delete this._attrs[k]; };
  StubElement.prototype.hasAttribute = function (k) { return k in this._attrs; };
  StubElement.prototype.addEventListener = function (t, fn) { (this._listeners[t] ||= []).push(fn); };
  StubElement.prototype.removeEventListener = function () {};
  StubElement.prototype.dispatchEvent = function () { return true; };
  // Some code calls getContext on an element it assumes is a canvas. Hand back
  // a throwaway offscreen context rather than null so it can't crash the load.
  StubElement.prototype.getContext = function () {
    if (!this._scratch) { this._scratch = new HTMLCanvas(false); this._scratch._setSize(8, 8); }
    return this._scratch.getContext();
  };
  StubElement.prototype.getBoundingClientRect = function () { return { left: 0, top: 0, right: 0, bottom: 0, width: 0, height: 0, x: 0, y: 0 }; };
  StubElement.prototype.focus = function () {};
  StubElement.prototype.blur = function () {};
  StubElement.prototype.click = function () {};
  // Never return null: in the real page these selectors always resolve, so the
  // engine dereferences the result without guarding. A stub keeps load alive.
  StubElement.prototype.querySelector = function () { return new StubElement("div"); };
  ...
  StubElement.prototype.scrollIntoView = function () {};
  Object.defineProperty(StubElement.prototype, "firstChild", { get() { return this.children[0] || null; } });
  Object.defineProperty(StubElement.prototype, "lastChild", { get() { return this.children[this.children.length - 1] || null; } });
  G.__sbxStubElement = StubElement;

  // ---- document -----------------------------------------------------------
  const elementsById = Object.create(null);
  const primaryCanvas = new HTMLCanvas(true);
  primaryCanvas._w = 1280; primaryCanvas._h = 720;
  G.__sbxPrimaryCanvas = primaryCanvas;

  function getElementById(id) {
    if (id === "game") return primaryCanvas;
    let el = elementsById[id];
    if (!el) { el = new StubElement("div"); el.id = id; elementsById[id] = el; }
    return el;
  }
  function createElement(tag) {
    if (("" + tag).toLowerCase() === "canvas") return new HTMLCanvas(false);
    return new StubElement(tag);
  }
  const body = new StubElement("body");
  const docEl = new StubElement("html");
  G.document = {
    body, documentElement: docEl, head: new StubElement("head"),
    ...
    querySelector: () => new StubElement("div"),
    querySelectorAll: () => [],
  ...
  // ---- localStorage (sdmc-backed JSON) ------------------------------------
  const LS_PATH = "sdmc:/switch/sandboxels/localStorage.json";
  let lsData = {};
  try { const raw = nx.readFile(LS_PATH); if (raw) lsData = JSON.parse(raw) || {}; } catch (e) {}
  function lsSave() { try { nx.writeFile(LS_PATH, JSON.stringify(lsData)); } catch (e) {} }
  G.localStorage = {
    getItem: (k) => (k in lsData ? lsData[k] : null),
    setItem: (k, v) => { lsData[k] = "" + v; lsSave(); },
    removeItem: (k) => { delete lsData[k]; lsSave(); },
    clear: () => { lsData = {}; lsSave(); },
    key: (i) => Object.keys(lsData)[i] || null,
    get length() { return Object.keys(lsData).length; },
  };

  // ---- timers + rAF (driven by bootstrap's main loop) ---------------------
  // We don't have a real event loop; bootstrap services these queues.
  const timers = [];        // {id, fn, due, interval}
  const rafCbs = [];
  let timerId = 1;
  G.setTimeout = function (fn, ms) {
    const id = timerId++; timers.push({ id, fn, due: G.__sbxNow() + (ms || 0), interval: 0 }); return id;
  };
  G.setInterval = function (fn, ms) {
    const id = timerId++; timers.push({ id, fn, due: G.__sbxNow() + (ms || 0), interval: ms || 16 }); return id;
  };
  G.clearTimeout = function (id) { const i = timers.findIndex(t => t.id === id); if (i >= 0) timers.splice(i, 1); };
  G.clearInterval = G.clearTimeout;
  G.requestAnimationFrame = function (fn) { rafCbs.push(fn); return rafCbs.length; };
  G.cancelAnimationFrame = function () {};
  G.queueMicrotask = function (fn) { try { fn(); } catch (e) {} };
  G.__sbxTimers = timers;
  G.__sbxRafCbs = rafCbs;
  // Service due timers; returns nothing. Called by bootstrap each loop tick.
  G.__sbxRunTimers = function (now) {
    for (let i = 0; i < timers.length; i++) {
      const t = timers[i];
      if (now >= t.due) {
        try { t.fn(); } catch (e) { nx && nx.print && nx.print("timer err: " + e); }
        if (t.interval > 0) t.due = now + t.interval;
        else { timers.splice(i, 1); i--; }
      }
    }
  };
  G.__sbxRunRaf = function () {
    if (!rafCbs.length) return;
    const cbs = rafCbs.splice(0, rafCbs.length);
    for (const cb of cbs) { try { cb(G.__sbxNow()); } catch (e) {} }
  };

  // ---- time ---------------------------------------------------------------
  const startTick = nx.ticks();
  const TICKS_PER_MS = 19200000 / 1000; // Switch system tick is 19.2 MHz
  G.__sbxNow = function () { return (nx.ticks() - startTick) / TICKS_PER_MS; };
  G.performance = { now: G.__sbxNow };
  G.Date = G.Date; // native
  G.requestIdleCallback = function (fn) { return G.setTimeout(fn, 1); };
  G.cancelIdleCallback = function () {};

  // ---- window / navigator / location --------------------------------------
  const noop = function () {};
  G.navigator = {
    userAgent: "Sandboxels-Switch", platform: "Switch", language: "en",
    languages: ["en"], onLine: false, vendor: "", maxTouchPoints: 10,
    clipboard: { writeText: () => Promise.resolve(), readText: () => Promise.resolve("") },
    serviceWorker: { register: () => Promise.reject(new Error("no sw")) },
    getGamepads: () => [],
  };
  G.location = {
    href: "https://sandboxels.local/", protocol: "https:", host: "sandboxels.local",
    hostname: "sandboxels.local", pathname: "/", search: "", hash: "",
    origin: "https://sandboxels.local", reload: noop, replace: noop, assign: noop,
  };
  G.screen = { width: 1280, height: 720, availWidth: 1280, availHeight: 720, orientation: { type: "landscape-primary" } };
  G.history = {
  ...
  if (typeof G.Intl === "undefined") {
    const ident = { format: (x) => "" + x, formatToParts: (x) => [{ type: "literal", value: "" + x }] };
    G.Intl = {
      RelativeTimeFormat: function () {
        return {
          format: (v, unit) => (v < 0 ? Math.abs(v) + " " + unit + " ago" : (v > 0 ? "in " + v + " " + unit : "now")),
          formatToParts: (v, unit) => [{ type: "literal", value: (v < 0 ? Math.abs(v) + " " + unit + " ago" : "in " + v + " " + unit) }],
        };
      },
      NumberFormat: function () { return { format: (n) => "" + n, formatToParts: (n) => [{ type: "integer", value: "" + n }] }; },
      DateTimeFormat: function () { return { format: (d) => "" + d, formatToParts: () => [] }; },
      Collator: function () { return { compare: (a, b) => (a < b ? -1 : a > b ? 1 : 0) }; },
      ListFormat: function () { return { format: (a) => (Array.isArray(a) ? a.join(", ") : "" + a) }; },
      PluralRules: function () { return { select: () => "other" }; },
      getCanonicalLocales: (l) => (Array.isArray(l) ? l : [l || "en"]),
    };
    void ident;
  }
  // Minimal URLSearchParams (query strings are always empty on the Switch).
  G.URLSearchParams = function (init) {
    ...
    if (typeof init === "string") {
      ...
      for (const k in init) pairs.push([k, "" + init[k]]);
    }
    return {
      get: (k) => { const p = pairs.find((x) => x[0] === k); return p ? p[1] : null; },
      getAll: (k) => pairs.filter((x) => x[0] === k).map((x) => x[1]),
      has: (k) => pairs.some((x) => x[0] === k),
      set: (k, v) => { const p = pairs.find((x) => x[0] === k); if (p) p[1] = "" + v; else pairs.push([k, "" + v]); },
      append: (k, v) => pairs.push([k, "" + v]),
      delete: (k) => { for (let i = pairs.length - 1; i >= 0; i--) if (pairs[i][0] === k) pairs.splice(i, 1); },
      forEach: (fn) => pairs.forEach(([k, v]) => fn(v, k)),
      keys: function* () { for (const p of pairs) yield p[0]; },
      values: function* () { for (const p of pairs) yield p[1]; },
      toString: () => pairs.map(([k, v]) => encodeURIComponent(k) + "=" + encodeURIComponent(v)).join("&"),
    };
  };

  // window is the global object itself in browsers.
  G.window = G;
  G.self = G;
  G.top = G;
  G.parent = G;
  ...
  G.focus = noop;
  G.blur = noop;
  G.alert = function (m) { nx && nx.print && nx.print("[alert] " + m); };
  G.confirm = function () { return false; };
  G.prompt = function (_m, d) { return d == null ? null : d; };
  G.print = G.print || noop;
  ...
  G.getComputedStyle = function () { return makeStyle(); };
  const winListeners = {};
  G.addEventListener = function (t, fn) { (winListeners[t] ||= []).push(fn); };
  G.removeEventListener = noop;
  G.dispatchEvent = function () { return true; };
  G.__sbxWinListeners = winListeners;

  // ---- Image / Audio / network stubs --------------------------------------
  // Images never load (textures off), so renderers fall back to vector draw.
  G.Image = function () {
    const img = { width: 0, height: 0, complete: false, onload: null, onerror: null, _src: "" };
    Object.defineProperty(img, "src", {
      get() { return img._src; },
      set(v) { img._src = v; if (typeof img.onerror === "function") try { img.onerror(); } catch (e) {} },
    });
    return img;
  };
  G.Audio = function () { return { play: () => Promise.resolve(), pause: noop, load: noop, addEventListener: noop, currentTime: 0, volume: 1 }; };
  G.fetch = function () { return Promise.reject(new Error("network unavailable")); };
  G.XMLHttpRequest = function () {
    return {
      open: noop, send: function () { if (typeof this.onerror === "function") this.onerror(); },
      setRequestHeader: noop, abort: noop, addEventListener: noop,
      readyState: 0, status: 0, responseText: "", response: null,
    };
  };
  G.URL = G.URL || function (u) { return { href: u, toString: () => u }; };
  if (!G.URL.createObjectURL) G.URL.createObjectURL = () => "blob:sandboxels";
  if (!G.URL.revokeObjectURL) G.URL.revokeObjectURL = noop;
  G.Blob = G.Blob || function (parts) { return { size: 0, type: "", _parts: parts }; };
  G.FileReader = function () {
    return { readAsText: noop, readAsDataURL: noop, readAsArrayBuffer: noop, addEventListener: noop, onload: null, result: null };
  };
  G.atob = G.atob || function (s) { return s; };
  G.btoa = G.btoa || function (s) { return s; };

  // ---- pre-engine globals -------------------------------------------------
  // index.html sets these in small <script> blocks the extractor skips; the
  // engine reads them at load time. (Not "standalone" -- no Steam/Discord host.)
  if (typeof G.standalone === "undefined") G.standalone = false;
  if (typeof G.standaloneType === "undefined") G.standaloneType = null;
  if (typeof G.currentversion === "undefined") G.currentversion = "1.13.2";
  if (typeof G.saveVersion === "undefined") G.saveVersion = 7;
  if (typeof G.requestFullScreen === "undefined") G.requestFullScreen = noop;
...
try {
		setTimeout(function() {
			...
			if (loadingP && loadingP.style.display !== "none") {
				loadingP.firstChild.textContent = "Failed to load. A mod is likely causing this.\n\n";
			}
		}, 5000);
		...
		isOnSite = location.hostname === "sandboxels.r74n.com" || location.hostname === "neal.fun";
...
POWDER: function(pixel) {
	...
	if (tryMove(pixel, pixel.x, pixel.y+1) !== true) {
		if (Math.random() < 0.5) {
			if (tryMove(pixel, pixel.x+1, pixel.y+1) !== true) {
				tryMove(pixel, pixel.x-1, pixel.y+1);
			}
		} else if (tryMove(pixel, pixel.x-1, pixel.y+1) !== true) {
			tryMove(pixel, pixel.x+1, pixel.y+1);
		}
	}
	doDefaults(pixel);
...
LIQUID: function(pixel) {
	if (pixel.start === pixelTicks) {return}
	if (pixel.charge !== undefined && pixel.charge && elements[pixel.element].behaviorOn !== undefined) {
		pixelTick(pixel);
		return;
	}
	let viscMove = true;
...
SUPERFLUID: function(pixel) {
	...
	if (!tryMove(pixel, pixel.x, pixel.y+1)) {
		...
		let newx = pixel.flipX ? pixel.x-1 : pixel.x+1;
		if (Math.random() < 0.5) {
...
GAS: function(pixel) {
	...
	if (moved === false) {
		...
		for (var i = 0; i < diagonalCoordsShuffle.length; i++) {
...
FLY: function(pixel, onHit) {
	let nx = pixel.flipX ? -1 : 1;
	let ny = Math.random() < 0.5 ? -1 : 1;
	let hit = false;
	...
	for (var i = 0; i < adjacentCoords.length; i++) {
		...
		if (!isEmpty(x,y,true)) {
			...
			if (newPixel.element === pixel.element) {
				let avg = ((pixel.moist||0) + (newPixel.moist||0))/2;
				pixel.moist = avg;
				newPixel.moist = avg;
			}
...
SEEDRISE: function(pixel) {
	if (Math.random() < 0.02) {
		...
		HEATGLOW: function(pixel,ctx) {
			...
			const max = elements[pixel.element].tempHigh;
			let ratio = ((pixel.temp/max*100)|0)/100;
			if (ratio < 0.2) return;
			if (ratio > 1) ratio = 1;
			ratio -= 0.2;
		...
		HUESHIFT: function(pixel,ctx) {
			if (pixel.origColor === undefined && pixel.color.match(/^rgb/)) {
				pixel.origColor = pixel.color.match(/\d+/g);
			}
			if (pixel.origColor === undefined || viewInfo[view].effects !== true) {
				drawDefault(ctx,pixel);
				return;
			}
			let hsl = RGBToHSL(pixel.origColor);
			hsl[0] += 0.05;
			hsl[0] -= Math.min(60,(pixelTicks-pixel.start)/0.5)/360;
		...
		BORDER: function(pixel,ctx) {
			...
			if (edge) { drawSquare(ctx,"rgb(255,255,255)",pixel.x,pixel.y, undefined, (pixel.alpha||1)/4) }
		},
...
"drag": {
	...
	tool: function(pixel) {
		...
		if (pixelTicks === dragStart && !pixel.drag && (elements[pixel.element].movable || shiftDown || pixel._r)) {
			pixel.drag = true;
	...
	perTick: function() {
		...
		for (var i = 0; i < draggingPixels.length; i++) {
			...
			for (let m = 0; m < moves; m++) {
				if (pixel.del) { break }
				if (pixel._r) {
					let relation = getRelation(pixel._r);
					if (relation.lastMove === pixelTicks) continue;
					let r = tryMoveRelation(relation, Math.sign(mousePos.x - pixel.x), Math.sign(mousePos.y - pixel.y));
					if (!r) {
						tryMoveRelation(relation, 0, Math.sign(mousePos.y - pixel.y), true);
						tryMoveRelation(relation, Math.sign(mousePos.x - pixel.x), 0, true);
					}
					continue;
				}
				const x = pixel.x;
...
"paint": {
	...
	tool: function(pixel) {
		...
		if (elements[pixel.element].trackPaint) pixel.painted = currentColorMap.paint;
	},
	customColor: true,
	category: "tools",
	canPlace: false,
...
"fire": {
	color: ["#ffb700","#ff8800"],
	buttonColor: ["#ff6b21","#ffa600","#ff4000"],
	tick: function(pixel){
		if (pixel.start === pixelTicks) {return}
		shuffleArray(adjacentCoordsShuffle);
		let moved = false;
		for (var i = 0; i < adjacentCoordsShuffle.length; i++) {
			...
			if (tryMove(pixel, x, y) === true) { moved = true; break; }
			if (!isEmpty(x,y,true) && pixelMap[x][y].element === "fire") {
				if (pixel.start < pixelMap[x][y].start && Math.random() < 0.1) {
					// pixelMap[x][y].temp += pixel.temp / 2;
					deletePixel(pixel.x, pixel.y);
				}
			}
		}
		if (moved === false) {
			...
			for (var i = 0; i < diagonalCoordsShuffle.length; i++) {
				const coords = diagonalCoordsShuffle[i];
				if (tryMove(pixel, pixel.x+coords[0], pixel.y+coords[1]) === true) break;
			}
		}
		if (pixel.del !== true) {
			doHeat(pixel)
	...
	onCollide: function(pixel1,pixel2) {
		if (elements[pixel2.element].fireColor !== undefined) {
...
"cold_fire": {
	color: "#009dff",
	buttonColor: ["#21cbff","#006aff","#00ffff"],
	tick: function(pixel){
		behaviors.GAS(pixel);
...
"battery": {
	...
	// ],
	onShiftSelect: function(element) {
		promptInput("Enter a number to use as the frequency. (Default 1)", function(r) {
			if (r === "") return;
			r = parseInt(r);
			if (isNaN(r)) {
				logMessage("Invalid integer - battery frequency not set.")
				return;
			};
			currentElementProp = { freq:r }
		}, elemTitleCase(elements[element].name || element));
	},
	tick: function(pixel) {
		doDefaults(pixel);
...
"random": {
	...
	tick: function(pixel) {
		...
		if (pixel.randomChoices) choice = choose(pixel.randomChoices.split(","));
		if (choice === "random") choice = choose(randomChoices);
...
"image": {
	color: ["#78bbff","#5bb81a"],
	onShiftSelect: function(element) {
		promptInput("Enter an element to make up the image.", function(r) {
			r = validateElementList(r);
			if (!r) return;
			currentElementProp = { imageelem:r }
		}, elemTitleCase(elements[element].name || element));
	},
	onSelect: function() {
...
"ruler": {
	...
	renderTool: function(ctx) {
		...
		let hypotCoords = lineCoords(newX, startY, newX, newY, 1);
		coords = coords.concat(hypotCoords);
		...
		if (triWidth > 5 && triHeight >= 5) { /* hypot */
			nums.push( [hypotCoords.length, (newX + startX) / 2 - dirX, (newY + startY) / 2 + dirY] );
		}
...
"group": {
	color: "#75ffef",
	onMouseDown: function() { editStart() },
	onMouseUp: function() {
		let pixels = editingPixels;
		editStop();
		...
		pixels.forEach(pixel => {
			if (pixel._r !== undefined) {
				...
				else removeFromRelation(pixel);
			}
		})
		if (!id) {
			id = currentRelations._id;
			currentRelations._id ++;
		}
		
		pixels.forEach(pixel => {
			addToRelation(pixel, id);
		})
		getRelation(id);
	},
	tool: function(pixel) {
		if (editingPixels.indexOf(pixel) === -1) {
			editingPixels.push(pixel);
		}
	},
	renderTool: function(ctx) {
		if (shiftDown || mouseIsDown) return;
		let pixel = getPixel(mousePos.x, mousePos.y);
		if (!pixel) return;
		if (pixel._r === undefined) return;
		let relation = getRelation(pixel._r);
		relation.p.forEach((p) => {
			drawSquare(ctx, mouseColor, p.x, p.y, undefined, 0.5);
		})
	},
	category: "edit",
...
"unpaint": {
	...
	tool: function(pixel) {
		...
		if (pixel.painted) delete pixel.painted;
	},
	reactions: {
	...
	tick: function(pixel) {
		...
		for (var i = 0; i < adjacentCoords.length; i++) {
			...
			if (isEmpty(x,y)) {
				...
				if (pixel.painted) {
					pixelMap[x][y].color = pixelColorPick(pixel, pixel.painted);
					pixelMap[x][y].painted = pixel.painted;
				}
			}
			else if (!outOfBounds(x,y)) {
				if (elements[pixelMap[x][y].element].id === elements.filler.id && pixelMap[x][y].painted !== pixel.painted) {
					deletePixel(x,y);
					createPixel(pixel.element, x, y);
					if (pixel.clone) pixelMap[x][y].clone = pixel.clone;
					if (pixel.painted) {
...
"worm": {
	...
	reactions: {
		...
		"rotten_cheese": { elem2:"dirt", chance:0.01, func:behaviors.FEEDPIXEL },
		...
		"dough": { elem2:"dirt", chance:0.01, func:behaviors.FEEDPIXEL },
		"paper": { elem2:"cellulose", chance:0.01 },
		"confetti": { elem2:"cellulose", chance:0.01 },
		"cellulose": { elem2:[null,"dirt"], chance:0.01, func:behaviors.FEEDPIXEL },
		"potato": { elem2:"mashed_potato", chance:0.01 },
		"mashed_potato": { elem2:"dirt", chance:0.01, func:behaviors.FEEDPIXEL },
		"mudstone": { elem2:"dirt", chance:0.1 },
		"adobe": { elem2:"dirt", chance:0.01, func:behaviors.FEEDPIXEL },
...
"fuse": {
	...
	hoverStat: function(pixel) { return pixel.clone ? "???" : "" },
	movable: true,
	tempHigh: 500,
	stateHigh: "fire",
	burn: 100,
...
"acid": {
	...
	ignore: ["fire","smoke","glass","rad_glass","glass_shard","rad_shard","stained_glass","baked_clay","acid_gas","neutral_acid","acid_cloud","water","salt_water","sugar_water","dirty_water","copper","gold","porcelain","plastic","bead","molten_plastic","pool_water","chlorine","hydrogen","gold_coin","silver","nickel","calcium","bone","earthquake","tornado","tsunami","liquid_light","sensor","clay","pipe","pipe_wall","filter","gate","lye","rose_gold","purple_gold","blue_gold","electrum","mercury","bubble","diamond"],
	reactions: {
...
"ecloner": {
	...
	onClicked: function(pixel,element) {
		if (element === "fuse") pixel.clone = "fuse";
		else behaviors.CLONE_ON_CLICK(pixel,element)
	},
...
"vinegar": {
	...
	reactions: {
		...
		"cream": { elem1: null, elem2: "cheese" },
		"pilk": { elem1: null, elem2: "cheese", color2:"#d6b57a" },
...
"bleach": {
	...
	reactions: {
		...
		"baking_soda": { elem1:null, elem2: "chlorine", chance: 0.01 },
		"blood": { elem2: null },
...
"chlorine": {
	...
	reactions: {
		...
		"hydrogen": { elem1: "acid_gas", elem2:null, tempMax:1000 },//hydrochloric acid
		"dirty_water": { elem2:"water" },
...
"dye": {
	...
	reactions: {
		...
		"sand": { func: function(pixel1,pixel2) {
			if (!pixel1.color.match(/^rgb/)) return;
			let hsl = RGBToHSL(pixel1.color.match(/\d+/g));
			pixel2.colorstart = Math.floor(hsl[0]*360 + Math.random()*10);
			changePixel(pixel2,"color_sand");
		} }
	},
	customColor: true,
	stain: 0.66,
...
"infection": {
	...
	behavior: behaviors.LIQUID,
	tick: function(pixel) {
		if (Math.random() < 0.001) { releaseElement(pixel,"plague"); }
	},
	reactions: {
		"blood": { elem2:"infection", chance:0.01 },
...
"honey": {
	...
	tempLow: 0,
	stateLow: "candy",
	stateLowColor: ["#fcedac","#fece80","#fdbf8b"],
	category:"liquids",
	state: "liquid",
	density: 1420,
	isFood: true
...
"molten_sodium": {
	...
	reactions: {
		...
		"water": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"salt_water": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"sugar_water": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"dirty_water": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"seltzer": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"pool_water": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"primordial_soup": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
		"nut_milk": { elem1:["pop","pop","pop","hydrogen","lye"], chance:0.01, temp2:250 },
...
"calcium": {
	...
	reactions: {
		...
		"steam": { elem1:["slaked_lime","pop"], elem2:["hydrogen","bubble"], chance:0.005, temp2:350 }
	},
	tempHigh: 842,
	category: "powders",
	state: "solid",
...
"magnesium": {
	...
	behavior: behaviors.POWDER,
	reactions: {
		"water": { elem1:null, elem2:"hydrogen", chance:0.01 },
		"steam": { elem1:null, elem2:"hydrogen", chance:0.05 },
		"salt_water": { elem1:null, elem2:"hydrogen", chance:0.01 },
		"sugar_water": { elem1:null, elem2:"hydrogen", chance:0.01 },
		"dirty_water": { elem1:null, elem2:"hydrogen", chance:0.01 },
		"seltzer": { elem1:null, elem2:"hydrogen", chance:0.01 },
		"pool_water": { elem1:null, elem2:"hydrogen", chance:0.01 },
		"nut_milk": { elem1:null, elem2:"hydrogen", chance:0.01 }
	},
	tempHigh: 650,
	category: "powders",
	state: "solid",
...
"lye": {
	...
	behavior: behaviors.POWDER,
	reactions: {
		"nut_oil": { elem1:"soap", elem2:null, chance:0.03 },
		"grease": { elem1:"soap", elem2:null, chance:0.03 },
		"fat": { elem1:"soap", elem2:null, chance:0.005 },
		"chlorine": { elem1:"bleach", elem2:null, chance:0.03 },
		"sodium_acetate": { elem1:"methane", elem2:null, chance:0.005 },
		"acid": { elem1:"salt_water", elem2:null, chance:0.005, temp1:500 },
		"aluminum": { elem1:"hydrogen", elem2:null, chance:0.002 },
		"lead": { elem1:"hydrogen", elem2:null, chance:0.005 },
		"molten_lead": { elem1:"hydrogen", elem2:null, chance:0.002 },
		"molten_tin": { elem1:"hydrogen", elem2:null, chance:0.002 },
		"tin": { elem1:"hydrogen", elem2:null, chance:0.002 },
		"zinc": { elem1:"hydrogen", elem2:null, chance:0.002 },
	...
	state: "solid",
	density: 2130,
	alias: ["sodium hydroxide","caustic soda"]
},
"molten_lye": {
	color: "#976c30",
	behavior: behaviors.LIQUID,
	reactions: {
		"acid_gas": { elem1:["steam","salt"], elem2:null, chance:0.01 },
		"aluminum": { elem1:"hydrogen", elem2:null, chance:0.002 },
		"molten_aluminum": { elem1:"hydrogen", elem2:null, chance:0.005 },
...
"portal_in": {
	...
	tick: function(pixel) {
		...
		if (ticktemp.portal_out[channel].length) {
			...
			for (var i = 0; i < squareCoordsShuffle.length; i++) {
				...
				var y = pixel.y+coord[1];
				if (!isEmpty(x,y,true) && elements[pixelMap[x][y].element].movable &&
					elements.portal_in.ignore.indexOf(pixelMap[x][y].element) === -1) {
					r = pixelMap[x][y];
					break;
				}
			}
	...
	renderTool: function(ctx) {
		let pixel = getPixel(mousePos.x, mousePos.y);
		if (pixel && (pixel.element === "portal_in" || pixel.element === "portal_out")) return;
		let portals = currentPixels.filter((p) => {
			if (!p) return;
			return elements[p.element].id === elements.portal_in.id;
		});
		if (!portals.length) return;
		let groups = categorizePixels(portals);
		if (!groups.length) return;
		let lines = 0;
		for (let i = 0; i < groups.length; i++) {
			elements.portal_in.hoverRender(ctx, groups[i][Math.floor(groups[i].length / 2)]);
			lines ++;
			if (lines > 30) return;
		}
	},
	hoverRender: function(ctx, pixel) {
		if (shiftDown || mouseIsDown) return;
		if (!pixel) pixel = getPixel(mousePos.x, mousePos.y);
		if (!pixel) return;
		if (pixel.channel === undefined) return;

		let portals = currentPixels.filter((p) => {
			if (!p) return;
			return elements[p.element].id === elements.portal_out.id && p.channel === pixel.channel;
		})
		if (!portals.length) return;
		...
		ctx.lineWidth = pixelSizeHalf;
		for (let i = 0; i < groups.length; i++) {
			const newPixel = groups[i][Math.floor(groups[i].length / 2)];
			ctx.strokeStyle = newPixel.color;
			ctx.globalAlpha = 0.5;
			ctx.beginPath();
			ctx.moveTo(canvasCoord(pixel.x + 0.5), canvasCoord(pixel.y + 0.5));
			ctx.lineTo(canvasCoord(newPixel.x + 0.5), canvasCoord(newPixel.y + 0.5));
			ctx.stroke();
		}
	},
...
"portal_out": {
	...
	renderTool: function(ctx) {
		...
		let lines = 0;
		for (let i = 0; i < groups.length; i++) {
			elements.portal_out.hoverRender(ctx, groups[i][Math.floor(groups[i].length / 2)]);
			lines ++;
			if (lines > 30) return;
		}
	},
	hoverRender: function(ctx, pixel) {
		...
		let portals = currentPixels.filter((p) => {
			if (!p) return;
			return elements[p.element].id === elements.portal_in.id && p.channel === pixel.channel;
		})
		if (!portals.length) return;
...
"copper_sulfate": {
	...
	reactions: {
		...
		"sugar_water": {elem1:"oxidized_copper", elem2:null, color1:["#CB3D3D","#A6292B","#6E1B1B"]},
		"caramel": {elem1:"oxidized_copper", elem2:null, color1:["#CB3D3D","#A6292B","#6E1B1B"]},
...
"radiation": {
	...
	reactions: {
		...
		"batter": { elem2:["baked_batter","rad_steam"], chance:0.5 },
...
"molten_caustic_potash": {
	...
	reactions: {
		// electrolysis:
		"aluminum": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.0025 },
		"zinc": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.015 },
		"steel": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.0125 },
		"iron": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.0125 },
		"tin": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.01 },
		"brass": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.001 },
		"bronze": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.001 },
		"copper": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.0075 },
		"silver": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.0075 },
		"gold": { elem1:["potassium","hydrogen","oxygen"], charged:true, chance:0.0075 }
	}
},
"sodium_acetate": {
	color: ["#ededed","#dbdbdb"],
...
"lightning": {
	color: "#ffffed",
	onPlace: function(pixel) {
		let coords = circleCoords(pixel.x, pixel.y, 10);
		for (let i = 0; i < coords.length; i++) {
			const coord = coords[i];
			let newPixel = getPixel(coord.x, coord.y);
			if (!newPixel) continue;
			if (elements[newPixel.element].colorOn) {
				pixel.color = pixelColorPick(pixel, elements[newPixel.element].colorOn);
				pixel.colored = true;
				return;
			}
			if (elements[newPixel.element].fireColor) {
				pixel.color = pixelColorPick(pixel, elements[newPixel.element].fireColor);
				pixel.colored = true;
				return;
			}
		}
		...
		if (!pixel.stage) { // create bolt
			...
			for (var i = 0; i < height - pixel.y + 1; i++) {
				...
				if (isEmpty(x, y)) {
					...
					pixelMap[x][y].color = pixel.color;
					if (pixel.colored === true) pixelMap[x][y].colored = true;
					last = [x,y];
				}
					...
					if (!outOfBounds(x,y) && !elements[pixelMap[x][y].element].insulate) { pixelMap[x][y].temp = 27760 }
					else if (currentSaveData.voidY) {
						break;
					}
	...
	tick: function(pixel) {
		...
		for (var y = pixel.y+1; y < height+1; y++) {
			...
			if (isEmpty(x, y)) {
				...
				if (pixelMap[x][y].burning) {
					delete pixelMap[x][y].burning;
					delete pixelMap[x][y].burnStart;
				}
...
"ember": {
	...
	tick: function(pixel) {
		if (settings.burn === 0 && Math.random() < 0.05) {
			deletePixel(pixel.x,pixel.y);
			return;
		}
		if (pixel.alpha === undefined) { pixel.alpha = 0.2 }
...
"tornado": {
	...
	firedColors: {
		...
		"midas_touch": ["#e3df6d","#d4ae6e","#e0c85a"]
	},
	tick: function(pixel) {
		doHeat(pixel);
		if (pixel.fired) {
	...
	reactions: {
		...
		"midas_touch": { attr1:{"fired":"midas_touch"} }
	},
	category: "weapons",
	state: "gas",
	density: 1.23,
...
"debug": {
	...
	tool: function(pixel) {
		...
		promptText(output,undefined,"Debug");
...
"prop": {
	...
	tool: function(pixel) {
		...
		if (typeof currentPropValue === "boolean" && typeof pixel[currentProp] === "number") {
			currentPropValue = currentPropValue ? 1 : 0;
		}
		else if (typeof currentPropValue === "number" && typeof pixel[currentProp] === "boolean") {
		...
		if (currentProp === "_r" && currentPropValue) {
			let relation = getRelation(currentPropValue);
			if (relation && relation.p.indexOf(pixel) === -1) {
				relation.p.push(pixel);
			}
		}
	},
	category: "edit"
...
elements.acid_gas.ignore = elements.acid.ignore;
...
queuedDependencies = [];
// Mod dependencies
function dependOn(modName, callback, forceLoad) {
	if (loadedMods.indexOf(modName) !== -1) {
		callback();
		return
	}
	if (!dependents[modName]) {
		dependents[modName] = [];
	}
	dependents[modName].push(callback);
	if (forceLoad && loadedMods.indexOf(modName) === -1 && queuedMods.indexOf(modName) === -1 && queuedDependencies.indexOf(modName) === -1) {
		queuedDependencies.push((modName.startsWith("http") ? "" : (standalone ? "https://sandboxels.r74n.com/mods/" : "mods/")) + modName);
	}
}
function checkDependents(modName) {
	if (loadedMods.indexOf(modName) !== -1) {
		return;
	}
	if (dependents[modName]) {
		for (let i = 0; i < dependents[modName].length; i++) {
			dependents[modName][i]();
		}
	}
}
function modIsEnabled(modName) {
	if (modName.indexOf("/") !== -1) {
		modName = modName.match(/\/[^\/]+$/g)[0].substring(1);
	}
	return enabledModNames.indexOf(modName) !== -1;
}
for (var i = 0; i < enabledMods.length; i++) {
	let src = enabledMods[i];
	let split = src.split("/");
	let modName = split[split.length-1];
	enabledModNames.push(modName);
}
if (standalone && window.initialData && window.initialData.modScripts) {
	for (let modName in window.initialData.modScripts) {
		let js = window.initialData.modScripts[modName];
		let script = document.createElement('script');
		script.innerHTML = js;
		document.head.appendChild(script);
	}
	delete window.initialData.modScripts;
}
...
for (var i = 0; i < enabledMods.length; i++) {
	try {
		...
		queuedMods.push(modName);
		script.src = src;
		script.addEventListener("load", function(){
			checkDependents(modName);
			loadedMods.push(modName);

			for (var i = 0; i < queuedDependencies.length; i++) {
				try {
					let script = document.createElement('script');
					let src = queuedDependencies[i];
					let split = src.split("/");
					let modName = split[split.length-1];
					if (src.startsWith("http")) {
						modName = src;
					...
					console.log("Error in mod: " + queuedDependencies[i]);
					console.log(e);
				}
			}
		})
...
function langKey(key,fallback,template) {
	...
	return lang[key];
		...
		function RGBToHex(rgb) {
			const r = parseInt(rgb.r||rgb[0]);
			const g = parseInt(rgb.g||rgb[1]);
			const b = parseInt(rgb.b||rgb[2]);
		...
		function averageRGB(rgblist) {
			...
			for (var i = 0; i < rgblist.length; i++) {
				...
				if (Array.isArray(rgb)) {
					newr = rgb[0], newg = rgb[1], newb = rgb[2];
				}
				else {
		...
		class Pixel {
			constructor(x, y, element) {
				...
				if (elementInfo.colorPattern && settings.textures!==0) {
					this.color = colorPatternPick(this);
				}
				else {
		...
		if (standalone && settings.pixelsize === undefined) {
			settings.pixelsize = standaloneType === "steam" ? "7w" : "6w";
		}
		...
		function outOfSight(x,y) {
			return y > height || y < 0 || x > width || x < 0
		}
		function onBorder(x,y) {
			return y === height-currentSaveData.border || y === currentSaveData.border || x === width-currentSaveData.border || x === currentSaveData.border
		}
		function onBorderY(y) {
			return y === height-currentSaveData.border || y === currentSaveData.border
		}
		function onBorderX(x) {
			return x === width-currentSaveData.border || x === currentSaveData.border
		}
		function isEmpty(x, y, ignoreBounds=false, oob=false) {
		...
		function deletePixel(x,y) {
			...
			// if (i !== -1) currentPixels.splice(currentPixels.indexOf(pixel),1);
			if (pixel) {
				pixel.del = true;
				if (elements[pixel.element].onDelete !== undefined) {
		...
		function swapPixels(pixel1,pixel2) {
			const tempX = pixel1.x;
			const tempY = pixel1.y;
			pixel1.x = pixel2.x;
			pixel1.y = pixel2.y;
			pixel2.x = tempX;
			pixel2.y = tempY;
		...
		function changePixel(pixel,element,changetemp=true) {
			...
			if (pixel._r !== undefined) {
				if (elements[pixel.element].state !== elements[element].state) {
					removeFromRelation(pixel);
				}
				else if (!elements[pixel.element].movable && elements[element].movable === true) {
					removeFromRelation(pixel);
				}
			}
			...
			if (elementInfo.temp !== undefined && changetemp === true) {
		...
		function reactPixels(pixel1,pixel2) {
			const r = elements[pixel1.element].reactions[pixel2.element];
			...
			if (pixel1.skip === true || pixel2.skip === true) { return false }
			if (r.setting !== undefined && settings[r.setting]===0) {
				return false;
			}
			...
			if (r.elem1 !== undefined) {
				...
				if (Array.isArray(r.elem1)) {
					elem1 = r.elem1[Math.floor(Math.random() * r.elem1.length)];
				} else { elem1 = r.elem1; }
			...
			if (r.charge1) {
				...
				if (elements[pixel1.element].colorOn) pixel1.color = pixelColorPick(pixel1);
			}
			...
			if (r.elem2 !== undefined) {
				...
				if (Array.isArray(r.elem2)) {
					elem2 = r.elem2[Math.floor(Math.random() * r.elem2.length)];
				} else { elem2 = r.elem2; }
			...
			if (r.charge2) {
				...
				if (elements[pixel2.element].colorOn) pixel2.color = pixelColorPick(pixel2);
			}
		...
		function tryCreate(element, x, y, replace) {
			if (!isEmpty(x,y)) {
				if (replace && !outOfBounds(x,y)) {
					deletePixel(x,y);
				}
				else return null;
			}
		...
		function chargePixel(pixel,amount=1) {
			const con = elements[pixel.element].conduct;
			if (con !== undefined) {
				if (Math.random() < con) { // If random number is less than conductivity
		...
		function floodPixels(x, y, check, limit, stopAt) {
			let checked = {};
			let toCheck = [[x,y]];
			let toCheckKeys = {};
			toCheckKeys[x+","+y] = true;
			let results = [];
			while (toCheck.length) {
				let pixel = getPixel(toCheck[0][0], toCheck[0][1]);
				let key = toCheck[0][0] + "," + toCheck[0][1];
				checked[key] = true;
				if (pixel && check(pixel)) {
					results.push(pixel);
					if (limit !== undefined && results.length >= limit) break;
					for (let i = 0; i < adjacentCoords.length; i++) {
						const coords = adjacentCoords[i];
						const newPixel = getPixel(pixel.x + coords[0], pixel.y + coords[1]);
						if (!newPixel) continue;
						const newKey = (pixel.x+coords[0]) + "," + (pixel.y+coords[1]);
						if (checked[newKey] !== undefined) continue;
						if (toCheckKeys[newKey] !== undefined) continue;
						if (stopAt !== undefined && stopAt(pixel)) {
							if (check(pixel)) results.push(pixel);
							continue;
						}
						toCheck.push([newPixel.x, newPixel.y]);
						toCheckKeys[newPixel.x + "," + newPixel.y] = true;
					}
				}
				toCheck.shift();
			}
		...
		function categorizePixels(pixels) {
			let groups = [];

			for (let i = 0; i < pixels.length; i++) {
				const pixel = pixels[i];
				if (pixel._CATDONE) continue;
				let flood = floodPixels(pixel.x, pixel.y, (p) => pixels.indexOf(p) !== -1);
				flood.forEach((p) => {
					p._CATDONE = true;
				})
				if (flood.length) groups.push(flood);
			}

			for (let i = 0; i < pixels.length; i++) {
				const pixel = pixels[i];
				delete pixel._CATDONE;
			}
		...
		function tryMove(pixel,nx,ny,leaveBehind,force) {
			if (pixel.drag === true && force !== true) return true;
			if (pixel.del === true) return false;
			...
			if (pixel._r !== undefined) {
				let relation = getRelation(pixel._r);
				if (relation.lastMove === pixelTicks) return;
				let dirX = Math.sign(nx - pixel.x);
				let dirY = Math.sign(ny - pixel.y);
				tryMoveRelation(relation, dirX, dirY);
				return;
			}
			...
			const infoReactions = info.reactions;
			if (oob === true) return false;
			
			// Reactions
			const newPixel = pixelMap[nx][ny];
			const newElement = newPixel.element;
			let rr1 = false;
			if (infoReactions !== undefined && infoReactions[newElement] !== undefined) {
				rr1 = reactPixels(pixel,newPixel)
				if (rr1 === true) {
					return true;
				}
			}
			const newPixelInfo = elements[newElement];
			const newPixelReactions = newPixelInfo.reactions;
			if (rr1 !== true && newPixelReactions !== undefined) {
				const reaction = newPixelReactions[pixel.element];
				if (reaction !== undefined && reaction.oneway !== true) {
					if (reactPixels(newPixel,pixel)) {
						return true;
					}
				}
			}
			if (info.id !== newPixelInfo.id) {
				// Density
				if (info.density !== undefined && newPixelInfo.density !== undefined) {
					...
					if (validDensitySwaps[info.state][newPixelInfo.state] !== undefined && info.density >= newPixelInfo.density && newPixel.skip !== true) {
						// chance depending on the difference in density
						if (Math.random() < (info.density - newPixelInfo.density)/(info.density + newPixelInfo.density)) {
							swapPixels(pixel,newPixel);
							return true;
						}
					}
				}
				if (newPixelInfo.onMoveInto !== undefined) {
					newPixelInfo.onMoveInto(newPixel,pixel);
				}
				if (info.onCollide !== undefined) {
					info.onCollide(pixel,newPixel);
				}
			}
		...
		function rotateBehavior(behavior,rotation) {
			...
			const check = behaviorCache[behavior.toString()+rotation];
		...
		function flipBehavior(behavior,axis) {
			...
			const check = behaviorCache[behavior.toString()+axis];
			if (check !== undefined) { return check; }
		...
		function getRelation(id) {
			if (currentRelations[id] === undefined) {
				currentRelations[id] = {
					p: currentPixels.filter((pixel) => {
						return pixel._r === id;
					})
				}
			}
		...
		function tryMoveRelation(relation, dirX, dirY, forceTry) {
			if (!relation.p || !relation.p.length) return false;
			if (relation.lastMove === pixelTicks && forceTry !== true) return false;
			relation.lastMove = pixelTicks;

			let possible = true;
			let toDisplace = [];

			for (let i = 0; i < relation.p.length; i++) {
				const pixel = relation.p[i];
				if (pixel.del) {
					relation.p.splice(i,1);
					i--;
					delete pixel._r;
					continue;
				}
				...
				const newPixel = getPixel(x, y);
				if (newPixel) {
					if (pixel._r === newPixel._r) {
						continue; //is possible
					}
					// // sink in liquids/gases
					if (validDensitySwaps[elements[pixel.element].state][elements[newPixel.element].state] === true &&
						elements[newPixel.element].density < elements[pixel.element].density
					) {
						toDisplace.push(newPixel);
						continue; //is possible
					}
					else if (dirY < 0 && newPixel._r === undefined && elements[newPixel.element].movable) {
						toDisplace.push(newPixel);
						continue; //is possible
					}
					possible = false;
			...
			let spots = [];
			if (toDisplace.length !== 0) {
				for (let i = 0; i < relation.p.length; i++) {
					const pixel = relation.p[i];
					spots.push([pixel.x, pixel.y]);
				}
			}
			for (let i = 0; i < relation.p.length; i++) {
				const pixel = relation.p[i];
			...
			if (toDisplace.length !== 0) {
				spots = spots.filter((coords) => {
					return pixelMap[coords[0]][coords[1]] === undefined;
				})
				shuffleArray(spots);
				for (let i = 0; i < toDisplace.length; i++) {
					const pixel = toDisplace[i];
					let coords = spots[i];
					// if (!coords) break;
					pixel.x = coords[0];
					pixel.y = coords[1];
					pixelMap[pixel.x][pixel.y] = pixel;
					// movePixel(pixel, coords[0], coords[1]);
				}
			}
			return true;
		}
		function addToRelation(pixel, id) {
			...
			let relation = getRelation(id);
			let index = relation.p.indexOf(pixel);
			if (index === -1) relation.p.push(pixel);
			pixel._r = id;
		...
		function checkRelation(relation) {
			if (relation.p !== undefined && relation.p.length === 0) deleteRelation(relation);
		}
		function deleteRelation(rid) {
			if (currentRelations[rid]) {
				let relation = getRelation(rid);
				delete currentRelations[rid];
				if (relation.p !== undefined) {
					relation.p.forEach((pixel) => {
						delete pixel._r;
					})
				}
			}
...
"XX": function() {}
	...
	for (var bx = 0; bx < behaviorby.length; bx++) {
		...
		if (btemp.move === true) {
			if (move1Spots.length > 0) {
				...
				for (let i = 0; i < move1Spots.length; i++) {
					const coords = move1Spots[i];
					var nx = coords.x;
					var ny = coords.y;
			...
			if (moved === false && move2Spots.length > 0) {
				shuffleArray(move2Spots);
				// While move2Spots is not empty
				for (let i = 0; i < move2Spots.length; i++) {
					var coords = move2Spots[i];
					var nx = coords.x;
					var ny = coords.y;
		...
		function doDefaults(pixel) {
			if (pixel.del === true) return;
			if (elements[pixel.element].insulate !== true) doHeat(pixel);
			doAirDensity(pixel);
			doBurning(pixel);
			doElectricity(pixel);
			if (elements[pixel.element].stain !== undefined) doStaining(pixel);
		}

		function doAirDensity(pixel) {
			if (pixel.del === true) return;
			var info = elements[pixel.element];
			if (info.density < airDensity && info.ignoreAir !== true && info.movable === true) {
		...
		function doBurning(pixel) {
			if (pixel.burning) { // Burning
				...
				for (var i = 0; i < adjacentCoords.length; i++) { // Burn adjacent pixels
					...
					if (!isEmpty(x,y,true)) {
						var newPixel = pixelMap[x][y];
						if (elements[newPixel.element].burn !== undefined && newPixel.burning !== true) {
							if (Math.floor(Math.random()*100) < elements[newPixel.element].burn) {
								burnPixel(newPixel);
							}
						}
						if (elements[newPixel.element].extinguish === true && elements[pixel.element].burning !== true) {
							pixel.burning = undefined;
							pixel.burnStart = undefined;
							return;
						}
				...
				if ((pixelTicks - pixel.burnStart > (info.burnTime || 200)) && Math.floor(Math.random()*100)<(info.burn || 10) && !(info.burnTime === undefined && info.hardness >= 1)) {
					...
					if (info.fireColor !== undefined && makesFire) {
				...
				else if (Math.random() < 0.1 && info.id !== elements.fire.id && info.fireElement !== null) { // Spawn fire
					if (isEmpty(pixel.x,pixel.y-1)) {
		...
		function doHeat(pixel) {
			// const weight1 = elements[pixel.element].heatCapacity || 1;
			let changed = false;
			// Check right and bottom adjacent pixels
			for (var i = 0; i < biCoords.length; i++) {
				const x = pixel.x+biCoords[i][0];
				const y = pixel.y+biCoords[i][1];
				if (isEmpty(x,y,true) === false) {
					...
					const avg = (pixel.temp + newPixel.temp)/2;
					// const weight2 = elements[newPixel.element].heatCapacity || 1;
		...
		function doElectricity(pixel,step) {
			if (pixel.chargeStart === pixelTicks && !step) return;
			if (pixel.charge) {
				// pixel.charge -= 0.2;
		...
		function doStaining(pixel) {
			...
			for (var i = 0; i < adjacentCoords.length; i++) {
				...
				if (!isEmpty(x,y,true)) {
					const newPixel = pixelMap[x][y];
					if (
						(elements[newPixel.element].id !== elements[pixel.element].id || elements[newPixel.element].stainSelf === true) &&
						(solidStates[elements[newPixel.element].state] || elements[newPixel.element].id === elements[pixel.element].id)
					) {
						if (elements[pixel.element].ignore && elements[pixel.element].ignore.indexOf(newPixel.element) !== -1) {
							continue;
						}
						if (Math.random() < stainAbs) {
							...
							else if (newPixel.origColor === undefined) {
		...
		function stainPixel(newPixel,color,intensity) {
			...
			let rgb = newPixel.color.match(/\d+/g);
		...
		function colorPatternPick(pixel) {
			const elementInfo = elements[pixel.element];
			return pixelColorPick(pixel, elementInfo.colorPattern[pixel.y%elementInfo.colorPattern.length][pixel.x%elementInfo.colorPattern[0].length]);
		}
		function pixelTempCheck(pixel) {
			...
			if (pixel.temp >= elementInfo.tempHigh) {
				...
				if (elementInfo.extraTempHigh !== undefined) {
				...
				if (result === null) {deletePixel(pixel.x,pixel.y);return false}
					...
					if (elements[result].customColor !== undefined) {
						var tempColor = pixel.color;
						changePixel(pixel,result,false);
						pixel.color = tempColor;
					}
					else if (elementInfo.stateHighColor !== undefined) {
						changePixel(pixel,result,false);
						pixel.color = pixelColorPick(pixel,elementInfo.stateHighColor);
					}
					else if (elementInfo.stateHighColorMultiplier !== undefined) {
						var color = pixel.color;
						changePixel(pixel,result,false);
					...
					if (elementInfo.fireColor !== undefined && result === "fire") {
						pixel.color = pixelColorPick(pixel,elementInfo.fireColor);
					}
					if (elementInfo.onStateHigh !== undefined) {
			...
			// If the pixel's temp <= the elementInfo tempLow, change pixel.element to elementInfo.stateLow
				...
				if (elementInfo.extraTempLow !== undefined) {
				...
				if (result === null) {deletePixel(pixel.x,pixel.y);return false}
					...
					else if (elementInfo.stateLowColor !== undefined) {
						changePixel(pixel,result,false);
						pixel.color = pixelColorPick(pixel,elementInfo.stateLowColor);
					}
					else if (elementInfo.stateLowColorMultiplier !== undefined) {
						var color = pixel.color;
						changePixel(pixel,result,false);
						var rgb = color.match(/\d+/g);
					...
					if (elementInfo.onStateLow !== undefined) {
	   ...
	   function selection(x1,y1,x2,y2) {
		...
		function generateScreenshot() {
			...
			drawLayers(shiftDown ? false : true);
		...
		vitalSettings = ["abszero","airdensity","airtemp","bg","burn","clouds","events","stain","limitless"];
		function uuidv4() {
		...
		function generateSave(pixelarray,saveOptions) {
			...
			for (var i = 0; i < pixelarray.length; i++) {
				for (var j = 0; j < pixelarray[i].length; j++) {
					...
					if (pixel) {
						var keepColor = false;
						if (elements[pixel.element].singleColor || elements[pixel.element].customColor || elements[pixel.element].forceSaveColor || pixel.painted !== undefined) {
			...
			if (standalone) {
				var url = standaloneType || "standalone";
			}
			...
			let relations = JSON.parse(JSON.stringify(currentRelations));
			for (let rid in relations) {
				delete relations[rid].p;
			}
			let saveConfig = {
			...
			if (currentSaveData.voidX) saveConfig.voidX = true;
			if (currentSaveData.voidY) saveConfig.voidY = true;
			if (currentSaveData.loopX) saveConfig.loopX = true;
			if (currentSaveData.loopY) saveConfig.loopY = true;
			return {
				"meta":{
		...
		function loadSave(saveJSON, confirmed=0, skip=0) {
			...
			if (!compressedMap) {
				promptText("Invalid save file - missing pixelMap.", undefined, "Error"); return;
			}
			var saveConfig = saveJSON.saveConfig || {};
			var savePixelSize = parseInt(saveConfig.pixelSize)||6;
			var version = parseInt(saveJSON.meta.saveVersion.split("sb")[1]);
			if (!confirmed) {
				if (version < 1 || !version) {
					promptConfirm("This save was made in an unknown version of Sandboxels, and may not load correctly.", (r)=>{ if (!r) return; loadSave(saveJSON,1,skip) }, "Warning");
					return;
				}
				else if (version > saveVersion) {
					promptConfirm("This save was made in a newer version of Sandboxels, and may not load correctly. Please use the latest version.", (r)=>{ if (!r) return; loadSave(saveJSON,1,skip) }, "Warning");
					return;
				}
				if (confirmed < 2 && saveConfig.height*saveConfig.width >= 1000000) {
					promptConfirm("This save is very large. Are you sure you want to load it?", (r)=>{ if (!r) return; loadSave(saveJSON,2,skip) }, "Warning");
					return;
				}
			}
			if (saveConfig.mods && skip < 3) {
				if (confirmed < 3) {
					for (var i = 0; i < saveConfig.mods.length; i++) {
						if (!modIsEnabled(saveConfig.mods[i])) {
							promptConfirm("Install included mods for this save? Only continue if you trust this file's source. They may or may not be necessary.\n\n"+saveConfig.mods.join("\n"), (r)=>{ if (!r) loadSave(saveJSON,confirmed,3); else loadSave(saveJSON,3,skip); }, "Warning");
							return;
						}
					}
				}
					...
					for (var i = 0; i < saveConfig.mods.length; i++) {
						if (!modIsEnabled(saveConfig.mods[i])) {
							let mod = addMod(saveConfig.mods[i],true);
							if (mod) added.push(mod);
						}
					}
					if (added.length === 0) loadSave(saveJSON,confirmed,3);
					localStorage.setItem("SandboxelsTempSave", JSON.stringify(saveJSON));
					localStorage.setItem("SandboxelsTempMods", JSON.stringify(added));
					window.location.reload();
					return;
				}
			}
			resizeCanvas(saveConfig.height*savePixelSize+savePixelSize,saveConfig.width*savePixelSize+savePixelSize,savePixelSize,true,true);
			clearAll(true);
			...
			let oldTps = tps;
			tps = parseInt(saveConfig.tps)||30;
			resetInterval(tps);
			...
			currentRelations = saveJSON.relations || { _id:1 };
			for (var x = 0; x < pixelarray.length; x++) {
				for (var y = 0; y < pixelarray[x].length; y++) {
					...
					if (pixel) {
						...
						if (pixel._r && currentRelations[pixel._r]) {
							let relation = currentRelations[pixel._r];
							if (!relation.p) relation.p = [];
							relation.p.push(pixelMap[x][y]);
						}
					}
					else {
			...
			pixelTicks = parseInt(saveConfig.pixelTicks)||0;
			...
			currentSaveData = {
				...
				"tags":saveJSON.meta.tags,
				"border":saveConfig.border,
				"voidX":saveConfig.voidX,
				"voidY":saveConfig.voidY,
				"loopX":saveConfig.loopX,
				"loopY":saveConfig.loopY,
				"oldTps":oldTps
			}
			if (saveJSON.meta.author) {
				logMessage("Created by "+saveJSON.meta.author);
			}
			if (showingMenu) closeMenu();
			return pixelarray;
		}
		...
		function breakPixel(pixel) {
			...
			if (pixel._r !== undefined && result !== undefined) {
				removeFromRelation(pixel);
			}
		}
		...
		diagonalCoordsShuffle = diagonalCoords.slice();
		...
		function clearLayers() {
			...
			for (let i = 0; i < canvasLayerKeys.length; i++) {
				...
				if (layerCanvas === canvasLayers.pixels) {
					if (viewInfo[view].clear === false) continue;
					if (!paused && pixelTicks === lastPixelDraw) continue;
				}
		...
		function drawLayers(includeBackground) {
			...
			if (includeBackground) {
				ctx.fillStyle = settings.bg || "#000000";
			...
			if (!includeBackground) drawCursor();
		...
		function drawCursor() {
			...
			if (settings.precision) {
				...
				layerCtx.fillRect(mousePos.x*pixelSize - (mouseSize % 2 ? 0 : pixelSize), mousePos.y*pixelSize - (mouseSize % 2 ? 0 : pixelSize), pixelSize + (mouseSize % 2 ? 0 : pixelSize), pixelSize + (mouseSize % 2 ? 0 : pixelSize));
			}
			if (shaping) {
			...
			if (editingPixels && editingPixels.length) {
				editingPixels.forEach((pixel) => {
					drawSquare(layerCtx,mouseColor,pixel.x,pixel.y,undefined,0.5);
				})
			}
		}
		function tickPixels() {
			...
			for (let i = 0; i < newCurrentPixels.length; i++) {
				const pixel = newCurrentPixels[i];
				const info = elements[pixel.element];
				if (pixel.skip === true) continue;
				if (pixel.del === true) continue;
				if (info.movable === true) {
					if (currentSaveData.voidX === true && onBorderX(pixel.x)) {
						deletePixel(pixel.x,pixel.y); continue;
					}
					if (currentSaveData.voidY === true && onBorderY(pixel.y)) {
						deletePixel(pixel.x,pixel.y); continue;
					}
					if (currentSaveData.loopX === true) {
						if (pixel.x === currentSaveData.border) tryMove(pixel,width-currentSaveData.border-1,pixel.y);
						else if (pixel.x === width-currentSaveData.border) tryMove(pixel,currentSaveData.border+1,pixel.y);
					}
					if (currentSaveData.loopY === true) {
						if (pixel.y === currentSaveData.border) tryMove(pixel,pixel.x,height-currentSaveData.border-1);
						else if (pixel.y === height-currentSaveData.border) tryMove(pixel,pixel.x,currentSaveData.border+1);
					}
				}
				if (pixel.del === true) continue;
				if (info.tick !== undefined) { // Run tick function if it exists
					info.tick(pixel);
				}
				if (pixel.del === true) continue;
				if (runPerPixelList.length !== 0) {
				...
				if (info.behavior !== undefined) { // Parse behavior if it exists
					pixelTick(pixel);
				}
			}

			for (let rid in currentRelations) {
				let relation = currentRelations[rid];
				if (relation.p !== undefined) {
					if (relation.p.length === 0) {
						delete currentRelations[rid];
						continue;
					}
					if (relation.lastMove === pixelTicks) continue;
			...
			1: { // Normal View (Default)
				...
				pixel: function(pixel,ctx) {
					...
					if (pixel.charge && view !== 2) { // Yellow glow on charge
						if (!elements[pixel.element].colorOn) {
							drawSquare(ctx,"rgb(255,255,0)",pixel.x,pixel.y,undefined,0.5);
						}
					}
				}
			},
			2: { // Thermal View
				...
				onSelect: function(ctx) {
					...
					currentPixels.forEach((p) => {
						if (elements[p.element].insulate) return;
						if (p.temp > thermalMax) thermalMax = (p.temp + thermalMax)/2;
						if (p.temp < thermalMin) thermalMin = (p.temp + thermalMin)/2;
					})
				},
				pixel: function(pixel,ctx) {
					var temp = pixel.temp;
					var hue = 255 - Math.round((temp - thermalMin) / (thermalMax - thermalMin) * 255);
					if (hue < 0) {hue = 0}
					if (hue > 225) {hue = 225}
		...
		_RGBToHexCache = {};
		function RGBToHexCache(rgbstring) {
			if (_RGBToHexCache[rgbstring] !== undefined) return _RGBToHexCache[rgbstring];
			const match = rgbstring.match(/[\d\.]+/g);
			if (match === null) return rgbstring;
			const color = RGBToHex(rgbstring.match(/[\d\.]+/g));
			_RGBToHexCache[rgbstring] = color;
			return color;
		}
		_HSLToHexCache = {};
		function HSLToHexCache(hslstring) {
			if (_HSLToHexCache[hslstring] !== undefined) return _HSLToHexCache[hslstring];
			const match = hslstring.match(/[\d\.]+/g);
			if (match === null) return hslstring;
			const color = RGBToHex(HSLtoRGB([match[0]/360,match[1]/100,match[2]/100]));
			_HSLToHexCache[hslstring] = color;
			return color;
		}
		function drawSquare(ctx,color,x,y,scale=1,opacity=1) {
			// console.log(color);
			if (color.charCodeAt(0) === 114) color = RGBToHexCache(color);
			ctx.fillStyle = color;
		...
		function drawPlus(ctx,color,x,y,scale=1,opacity=1) {
			// console.log(color);
			if (color.charCodeAt(0) === 114) color = RGBToHexCache(color);
			else if (color.charCodeAt(0) === 104) color = HSLToHexCache(color);
			ctx.fillStyle = color;
			opacity = 0.5*opacity;
			if (ctx.globalAlpha !== opacity) ctx.globalAlpha = opacity;
		...
		function currentPixelsFilterTest(i) {
			return i !== undefined;
		}
		function drawPixels(forceTick=false) {
			...
			for (let i = 0; i < currentPixels.length; i++) {
				...
				if (pixel.del === true) {
					if (pixelMap[pixel.x][pixel.y] === pixel) pixelMap[pixel.x][pixel.y] = undefined;
					// currentPixels.splice(i,1);
					currentPixels[i] = undefined;
					hasDeleted = true;
					// i--;
					continue;
				}
				if (pixel.con) {
					...
					else if (elements[pixel.element].canContain === true) {
						pixel = pixel.con;
					}
				}
				if (elements[pixel.element].isGas === true || elements[pixel.element].glow === true) pixelsLast.push(pixel);
				else pixelsFirst.push(pixel);
			}
			if (hasDeleted === true) {
				currentPixels = currentPixels.filter(currentPixelsFilterTest);
			}
			...
			if (!hiding) {
				...
				if (viewInfo[view].pixel) {
					for (let i = 0; i < pixelDrawList.length; i++) {
						...
						if (elements[pixel.element].renderer && (settings.textures !== 0 || viewInfo[view].fancy === false)) {
		...
		width = undefined;
		function tick() {
			if (mouseIsDown !== false && shaping === 0) {
				mouseAction(null,mousePos.x,mousePos.y);
			}

			if (paused === true || manualPaused === true) return;

			if (settings.events !== undefined && settings.events !== 0) {
				doRandomEvents();
			}
			if (elements[currentElement].perTick !== undefined) {
				elements[currentElement].perTick();
			}
			
			tickPixels();

			if (runEveryTickList.length !== 0) {
		...
		function defaultSaveData() {
			return {
				...
				border: 0,
				uuid: uuidv4(),
				voidX: settings.voidX,
				voidY: settings.voidY,
				loopX: settings.loopX,
				loopY: settings.loopY
			}
		}
		...
		function placeImage(placementX,placementY,scale) {
			...
			if (currentElementProp && currentElementProp.imageelem) {
				elem = currentElementProp.imageelem;
			}
		...
		function mouse1Action(e,mouseX=undefined,mouseY=undefined,startPos) {
			...
			for (var i = 0; i < coords.length; i++) {
				...
				if (outOfSight(x,y)) { continue; }
				
				var existingPixel = null;
				if (!isEmpty(x,y,true)) { existingPixel = pixelMap[x][y] }
				...
				if (mode === "replace" && (!elements[currentElement].tool || elements[currentElement].canPlace)) {
					...
					if (existingPixel !== null) {
						if (!(currentElement === existingPixel.element && existingPixel.start === pixelTicks-1)) {
							deletePixel(x,y);
						}
					}
				}
		...
		function mouse2Action(e,mouseX=undefined,mouseY=undefined,startPos) {
			...
			for (var i = 0; i < coords.length; i++) {
				...
				if (!isEmpty(x, y)) {
					...
					if (elements.erase) elements.erase.tool(pixelMap[x][y])
				}
			}
		}
		function mouseMiddleAction(e, mouseX, mouseY) {
			...
			if (!isEmpty(mouseX, mouseY,true)) {
				...
				if (elements.pick && elements.pick.tool) elements.pick.tool(pixel);
			}
		}
		...
		function chooseElementPrompt() {
			if (promptState && promptState.id === "elem") {
				delete promptState.id;
				closeMenu();
				return;
			}
			promptInput("Enter the element's ID or name.",
				(e) => {
					if (!e) { return; }
		...
		function checkPause() {
			...
			if (paused || manualPaused) pauseButton.setAttribute("on","true");
			else pauseButton.setAttribute("on","false");

			if (manualPaused && !paused) paused = true;
		}
		...
		function resetPrompt(confirmed=false) {
			if (confirmed || settings.resetwarning === 0 || currentPixels.length===0) {
				if (changedMods) {
					forceReload = true;
					location.reload();
					return;
		...
		function toggleEditMode() {
			...
			let editDiv = document.getElementById("category-edit");
			let categoriesDiv = document.getElementById("categoryControls");
			let elementsDiv = document.getElementById("elementControls");
			if (editMode) {
		...
		function selectElement(element) {
			...
			if (elements[element].locked === true) {
				logMessage("This element is locked.");
				return;
			}
			...
			if (editMode && elements[element].category !== "edit" && elements[element].category !== "tools" && element !== "unknown") {
				toggleEditMode();
			}
			else if (!editMode && elements[element].category === "edit") {
				toggleEditMode();
			}
			checkMouseSize();
		}
		...
		function mostSimilarElement(s) {
			...
			for (var e in elements) {
				if (elements[e].locked === true) continue;
		...
		function setView(n) {
			if (viewInfo[view] && viewInfo[view].onUnselect) {
				const canvas = canvasLayers.pixels;
				const ctx = canvas.getContext("2d");
				viewInfo[view].onUnselect(ctx);
			}
			if (!viewInfo[n]) { // reset view
				n = 1;
			}
			if (viewInfo[n].onSelect) {
				const canvas = canvasLayers.pixels;
				const ctx = canvas.getContext("2d");
				viewInfo[n].onSelect(ctx);
			}
			view = n;
		}
		...
		function createElementButton(element) {
			...
			let colorObjects = elements[element].colorObject;
			if (!Array.isArray(colorObjects)) {
				colorObjects = [colorObjects];
			}
			if (elements[element].buttonColor) {
				if (Array.isArray(buttonColor)) {
					colorObjects = [];
					for (let i = 0; i < buttonColor.length; i++) {
						const hex = buttonColor[i];
						colorObjects.push(hexToRGB(hex));
					}
				}
			...
			if (elements[element].locked) button.classList.add("disabled");
		...
		function createCategoryDiv(category) {
			...
			categoryButton.innerText = (lang[category] || category).replace(".","   ").replace(/_/g," ").replace(/\w\S*/g, function(txt){return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();}).replace("   ",".").replace(/ /g, "");
		...
		function setThemeElement(element) {
			...
			let color = elements[element].colorObject;
		...
		function setTheme(hex) {
			...
			let rgb;
			if (typeof hex === "string") rgb = hexToRGB(hex);
			else rgb = hex;
	...
	--theme-dark: rgb(${r/2}, ${g/2}, ${b/2}); /* color/2 */

	--theme-darker: rgb(${r/5}, ${g/5}, ${b/5}); /* color/5 */
	--theme-darker-opac85: rgba(${r/5}, ${g/5}, ${b/5}, 0.85);

	--theme-darkest: rgb(${Math.max(0,r-127)}, ${Math.max(0,g-127)}, ${Math.max(0,b-127)}); /* color-127 */
	--theme-darkest2: rgb(${Math.max(0,r-191)}, ${Math.max(0,g-191)}, ${Math.max(0,b-191)}); /* color-191 */

	--theme-opac85: rgba(${rgb}, 0.85);
	--theme-opac75: rgba(${rgb}, 0.75);
	--theme-opac50: rgba(${rgb}, 0.5);
	--theme-opac25: rgba(${rgb}, 0.25);
	--theme-opac10: rgba(${rgb}, 0.1);
}`;
			let styleTag = document.getElementById("themeStyle");
			if (!styleTag) {
		...
		function clearAll() {
			...
			if (currentSaveData.oldTps) setTPS(currentSaveData.oldTps);
			if (currentSaveData.loaded && location.search) {
...
for (var k = 0; k < b0.split(" AND ").length; k++) {
	...
	if (b == "M1" || b == "M2") {
		...
		if ((info.category == "tools" || info.category == "edit" || info.tool) && !info.canPlace) { infoText.innerHTML += "\nTool."}
		else {
	...
	if (error) { infoSearch.style.backgroundColor = "rgb(100, 33, 33)"; }
		...
		infoSearch.style.backgroundColor = "";
	...
	if (infoHistory.length > 1) {
		...
		function handleTab(elem) {
			...
			let currentTab = parent.querySelector('div.menuTab.selected');
			let newTab = parent.querySelector('div.menuTab[data-tab='+tabID+']');
			
			if (newTab == currentTab) return;

			let currentButton = parent.querySelector('button.menuTab[data-tab='+currentTab.getAttribute("data-tab")+']');
		...
		function addMod(url,noMessage) {
			...
			if (url.indexOf("/") == -1 && url.indexOf(".") == -1 && !noMessage) {
				promptText("Invalid mod URL.\n\nYou usually want something like 'survival.js' (without the quotes)", showModManager, "Error");
				return;
			}
			...
			else if (standalone && url.startsWith("mods/")) {
				url = "https://sandboxels.r74n.com/" + url;
			}
			...
			if (!noMessage) {
				changedMods = true;
				promptText((standalone ? "Reset the canvas" : "Refresh the page")+" to apply changes.", showModManager, "Added Mod");
			}
			return url;
		}
		function removeMod(url,noMessage) {
		...
		function saveMenuOptions(item) {
			if (item.workshop && standalone && !item.localFile) {
				return `
				<span class="saveSlotOptions">
					<span onclick="handleWorkshopAddClick(this);" data-workshop="${item.workshop}" class="saveOption saveOptionSave">${langKey("gui.saves.add","Add")}</span>
				</span>`
			}
			if (item.url && !item.localFile) {
				const url = item.url.replace(/"/g, "%22");
				return `
				<span class="saveSlotOptions">
					<span onclick="loadSaveURL(this.getAttribute('data-url'))" data-url="${url}" class="saveOption saveOptionLoad">${langKey("gui.saves.load","Load")}</span>
				</span>`
			}
			if (item.localFile && standalone) {
				const filename = (item.filename || item.name).replace(/"/g,"");
				if (item.workshop) {
					if (item.published) {
						return `
						<span class="saveSlotOptions">
							<span onclick="loadSaveFile(this.getAttribute('data-filename'))" data-filename="${filename}" class="saveOption saveOptionLoad">${langKey("gui.saves.load","Load")}</span>
							<span ${currentSaveData.filename !== item.filename ? "disabled=true" : ""} onclick="if (!this.getAttribute('disabled')) {overwriteSaveFile(this.getAttribute('data-filename'))} else {mustBeLoaded('overwrite')}" data-filename="${filename}" class="saveOption saveOptionSave">${langKey("gui.saves.save","Save")}</span>
							<span onclick="unpublishSaveFile(this.getAttribute('data-filename'), this.getAttribute('data-workshop'))" data-filename="${filename}" data-workshop="${item.workshop}" class="saveOption saveOptionClear">${langKey("gui.saves.unpublish","UnPublish")}</span>
							<span ${currentSaveData.filename !== item.filename ? "disabled=true" : ""} onclick="if (!this.getAttribute('disabled')) {republishSaveFile(this.getAttribute('data-filename'), this.getAttribute('data-workshop'))} else {mustBeLoaded('publish')}" data-filename="${filename}" data-workshop="${item.workshop}" class="saveOption saveOptionSave">${langKey("gui.saves.republish","RePublish")}</span>
						</span>`
					}
					return `
					<span class="saveSlotOptions">
						<span onclick="loadWorkshop(this.getAttribute('data-workshop'), this.getAttribute('data-filename'))" data-filename="${filename}" data-workshop="${item.workshop}" class="saveOption saveOptionLoad">${langKey("gui.saves.load","Load")}</span>
						<span onclick="deleteWorkshop(this.getAttribute('data-workshop'), this.getAttribute('data-filename'))" data-filename="${filename}" data-workshop="${item.workshop}" class="saveOption saveOptionClear">${langKey("gui.saves.delete","Delete")}</span>
					</span>`
				}
				return `
				<span class="saveSlotOptions">
					<span onclick="loadSaveFile(this.getAttribute('data-filename'))" data-filename="${filename}" class="saveOption saveOptionLoad">${langKey("gui.saves.load","Load")}</span>
					<span ${currentSaveData.filename !== item.filename ? "disabled=true" : ""} onclick="if (!this.getAttribute('disabled')) {overwriteSaveFile(this.getAttribute('data-filename'))} else {mustBeLoaded('overwrite')}" data-filename="${filename}" class="saveOption saveOptionSave">${langKey("gui.saves.save","Save")}</span>
					<span onclick="deleteSaveFile(this.getAttribute('data-filename'))" data-filename="${filename}" class="saveOption saveOptionClear">${langKey("gui.saves.delete","Delete")}</span>
					<span ${currentSaveData.filename !== item.filename ? "disabled=true" : ""} onclick="if (!this.getAttribute('disabled')) {publishSaveFile(this.getAttribute('data-filename'))} else {mustBeLoaded('publish')}" data-filename="${filename}" class="saveOption saveOptionSave">${langKey("gui.saves.publish","Publish")}</span>
				</span>`
			}

			return `
				<span class="saveSlotOptions">
					<span ${item.empty ? "disabled=true" : 'onclick="loadSlot('+item.slot+')"'} class="saveOption saveOptionLoad">${langKey("gui.saves.load","Load")}</span>
					<span onclick="saveSlot(${item.slot})" class="saveOption saveOptionSave">${langKey("gui.saves.save","Save")}</span>
					<span ${item.empty ? "disabled=true" : 'onclick="clearSlot('+item.slot+')"'} class="saveOption saveOptionClear">${langKey("gui.saves.clear","Clear")}</span>
				</span>`
		}
		function showSaves() {
			var savesParent = document.getElementById("savesParent");
			...
			var saveItems = [];
			for (var i = 1; i < 13; i++) {
				let save = localStorage.getItem("SandboxelsSaves/"+i);
				// var name = "Slot "+i;
				let name = langKey("guitemplate.saves.slot","Slot $1",{1:i})
				let time = null;
				if (save) {
					let parsed = JSON.parse(save);
					name = parsed.meta.name || langKey("gui.saves.unnamed","Unnamed");
					time = parsed.meta.updated || parsed.meta.created || null;
				}
		...
		function showOnlineWarning(callback) {
			if (settings.onlineWarning) {
				callback();
				return;
			}
			promptConfirm("Community-made saves are moderated, but some may not be appropriate for everyone. You must be 13 years old or older to view this content.\n\nConfirm if you are at least 13 years old.", (r) => {
				if (r) {
					settings.onlineWarning = true;
					saveSettings();
					callback();
				}
			},
		...
		function showFeaturedSaves() {
			if (document.getElementById("saveFeaturedList").getAttribute("done")) return;
			var saveItems = [];

			// https://raw.githubusercontent.com/R74nCom/Sandboxels-Featured-Saves/refs/heads/main/meta.json

			fetch("https://raw.githubusercontent.com/R74nCom/Sandboxels-Featured-Saves/refs/heads/main/meta.json").then(res => res.text())
			.then(res => {
				JSON.parse(res).forEach(info => {
					saveItems.push({
						name: info.file.replace(/\.sbxls$/,""),
						author: info.author,
						filename: info.file,
						url: "https://raw.githubusercontent.com/R74nCom/Sandboxels-Featured-Saves/refs/heads/main/saves/"+info.file
					})
				})

				displaySavesList("saveFeaturedList", saveItems, true);
			})
			.catch(error => {
				document.getElementById("saveFeaturedList").innerText = "ERROR: Could not connect";
			});

			document.getElementById("saveFeaturedList").setAttribute("done","true");
		}
		async function getDiscordSaves(clear) {
			const saveDiscordList = document.getElementById("saveDiscordList");
			if (clear) {
				saveDiscordList.setAttribute("data-page","1");
				saveDiscordList.removeAttribute("data-done");
			}
			else if (saveDiscordList.getAttribute("data-done")) return;
			const discordSearch = document.getElementById("discordSearch");
			let search = discordSearch.value.trim() || undefined;
			const discordFilter = document.getElementById("discordFilter");
			let tag = discordFilter.value || undefined;
			let page = parseInt(saveDiscordList.getAttribute("data-page")) || 1;
			let sort = "likesDesc";
			if (tag === "Recent") {
				sort = "newest";
				tag = undefined;
			...
			try {
				...
				saveDiscordList.innerText = "ERROR: Could not connect";
				return;
			}

			if (json.threads.length) {
				saveDiscordList.setAttribute("data-page",page + 1);
			}
			else {
				saveDiscordList.setAttribute("data-done","true");
			}

			json.threads.forEach((item) => {
				if (!item) return;
				if (!tag && !search && sort !== "newest" && item.tags && item.tags.includes("Modded")) {
					return;
				}
				saveItems.push({
					name: item.name,
					...
					viewLink: item.link,
					author: item.author,
					image: "https://sandboxels.live/browser/api/images/"+item.id
				})
			});

			displaySavesList("saveDiscordList", saveItems, clear);
			window.isLoadingDiscord = false;
		}
		function showDiscordSaves() {
			const discordFilter = document.getElementById("discordFilter");
			if (!discordFilter.getAttribute("data-loaded")) {
				for (let tagID in saveTags) {
					let tagName = tagID == "6" ? "Cooking" : saveTags[tagID];
					tagID = parseInt(tagID);
					if (tagID === 10 || tagID === 13 || tagID === 14) continue;
					discordFilter.insertAdjacentHTML("beforeend",
						`<option value="${tagName}">${tagName}</option>`
					);
				}
				discordFilter.setAttribute("data-loaded", "true");
			}

			getDiscordSaves(true);
		}

		function handleDiscordScroll(elem) {
			if (elem.scrollTop >= (elem.scrollHeight - elem.offsetHeight - 100)) {
				if (window.isLoadingDiscord) return;
				window.isLoadingDiscord = true;
		...
		window.discordSearchTimeout = null;
		window.discordSearchChange = function() {
			if (discordSearchTimeout) clearTimeout(discordSearchTimeout);

			discordSearchTimeout = setTimeout(discordSearchComplete, 500);
		}
		window.discordSearchComplete = function() {
			showDiscordSaves();
		}

		function displaySavesList(elemid,data,clear) {
			var saveList = document.getElementById(elemid);
			if (clear) saveList.innerHTML = "";
			let now = Date.now();
			for (let i = 0; i < data.length; i++) {
				const item = data[i];
				if (!item) continue;

				let name = item.name || langKey("gui.saves.unnamed","Unnamed");
				// truncate
				if (name.length > 32) { name = name.substring(0,32)+"..."; }
				let slot = item.slot || null;
				let url = item.url || null;
				let author;
				if (item.author) author = "by "+item.author;

				let time = item.time || null;
				let days = null;
				if (time) {
					let elapsed = now - time;
					days = rtf.format(-Math.round(elapsed/1000/24/60/60), "day");
				}

				let span = document.createElement("span");
				span.className = "saveSlot";
				if (slot) span.setAttribute("id","saveSlot"+slot);
				if (currentSaveData.filename && currentSaveData.filename === item.filename) span.classList.add("current")
				else if (currentSaveData.slot && currentSaveData.slot === item.slot) span.classList.add("current")
				span.setAttribute("data-name",name);
				
				let nameSpan = document.createElement("span");
				nameSpan.className = "saveSlotName";
				nameSpan.innerText = name;

				if (item.image) {
					let img = document.createElement("img")
					img.className = "saveSlotImage";
					img.src = item.image;
					span.appendChild(img);
				}

				if (item.url || item.viewLink) {
					let link = document.createElement("a");
					link.innerText = "View";
					link.className = "texticon";
					if (standalone && item.workshop) {
						link.setAttribute("href",item.viewLink || item.url);
						link.setAttribute("target","_blank");
						link.addEventListener("click", (e)=>{
							e.preventDefault();
							window.viewWorkshop(item.workshop);
							return false;
						})
						link.setAttribute("title","Open Workshop");
					}
					else {
						link.setAttribute("href",item.viewLink || item.url);
						link.setAttribute("target","_blank");
						link.setAttribute("title","Open external link");
					}
					nameSpan.insertAdjacentText("beforeend", " ")
				...
				if (subs) {
					...
					subSpan.className = "saveSlotSub";
					subSpan.innerText = subs.join(" • ");
					span.appendChild(subSpan);
				}

				span.insertAdjacentHTML("beforeend",saveMenuOptions(item));

				saveList.appendChild(span);
			}
		}
		function loadSaveURL(url) {
			promptText("Please hold :)",undefined,"Loading Save...");
			fetch(url).then(res => res.text())
			.then(res => {
				loadSave(JSON.parse(res));
				currentSaveData.filename = url.match(/\/[^\/]+$/g)[0].substring(1);

				// closeMenu();
			})
			.catch(error => {
				promptText("There was an error loading this save:\n\n"+error,undefined,"Error!");
			});
		}
		function saveSlot(i) {
			savingState = {slot:i};
		...
		function loadSlot(i) {
			...
			if (save) {
				...
				currentSaveData.slot = parseInt(i);
				closeMenu();
			}
		}
		...
		function confirmSave(confirmed=0) {
			...
			var config = {name:saveName,temp:saveTemp,mods:saveMods,settings:saveSettings,raw:saveRaw,keep:[],author:saveAuthor,desc:saveDesc,tags:[]};
			...
			if (standaloneType === "steam") {
				if (savingState.overwrite) {
					overwriteSaveFileConfirm(savingState.overwrite,generateSave(undefined,config))
				}
				else {
		...
		function loadFromFile() {
			...
			input.addEventListener("change", function(e) {
				...
				reader.onload = readerEvent => {
					...
					lastSaveJSON = content;
					loadSave(JSON.parse(content));
					currentSaveData.filename = file.name;
				}
				closeMenu();
			});
			input.click();
		...
		function showSavePrompt() {
			...
			var saveTagsSpan = document.getElementById("saveTagsSpan");
			if (saveTagsSpan.style.display !== "none") {
				var saveTagsList = document.getElementById("saveTags");
				saveTagsList.innerHTML = "";
				// <input type="button" value="Structure/City" data-tag="1" class="toggleInput" onclick="toggleInput(this,undefined,false)" state="0">
				for (let tagID in saveTags) {
					tagID = parseInt(tagID);
					if (tagID >= 90) continue;
					let tagName = saveTags[tagID];
					saveTagsList.insertAdjacentHTML("beforeend",`<input type="button" value="${tagName}" data-tag="${tagID}" class="toggleInput" onclick="toggleInput(this,undefined,false)" state="0">`)
				}
				if (currentSaveData.tags) {
					currentSaveData.tags.forEach((tagID) => {
						let tagElem = saveTagsList.querySelector('input[data-tag="'+tagID+'"]');
						if (tagElem) tagElem.setAttribute("state","1");
					})
				}
			}
		...
		function generateBorder(element) {
			...
			for (let i = 0; i < coords.length; i++) {
				...
				if (element !== null) createPixel(element, x, y);
			}
		}
		function updateBorder() {
			let d = currentSaveData;
			d.voidX = false; d.voidY = false; d.loopX = false; d.loopY = false;
			if (settings.border && isNaN(settings.border)) {
				d.voidX = true; d.voidY = true;
				generateBorder(settings.border);
				currentSaveData.borderElem = true;
			}
			else if (currentSaveData.borderElem) {
				currentSaveData.borderElem = false;
				generateBorder(null);
			}
			if (settings.border === 1) { d.voidX = true; d.voidY = true; }
			else if (settings.border === 2) { d.loopX = true; d.loopY = true; }
			else if (settings.border === 3) { d.loopX = true; }
		}
		...
		function promptConfirm(text, handler, title, danger) {
			let pause = false;
			if (promptState) { pause = promptState.wasPaused }
			else if (paused) { pause = true }
			promptState = {
		...
		function promptChoose(text, choices, handler, title) {
			let pause = false;
			if (promptState) { pause = promptState.wasPaused }
			else if (paused) { pause = true }
			promptState = {
		...
		function promptDir(text, handler, title) {
			let pause = false;
			if (promptState) { pause = promptState.wasPaused }
			else if (paused) { pause = true }
			promptState = {
...
function autoGen(newname,element,autoType) {
	...
	if (!elements[newname]) { elements[newname] = newelem; }
		...
		if (elements[newname].color) {
			delete newelem.color;
			delete newelem.colorObject;
		}
	...
	finalizeElement(newname,elements[newname]);
}
function checkAutoGen(key,obj) {
	if (obj.tempHigh!==undefined && (obj.stateHigh===undefined||obj.forceAutoGen)) {
		var newname = obj.stateHighName;
		if ((obj.state==="solid" || !obj.state)) { // Melting
			if (!newname) { newname = "molten_"+key }
			autoGen(newname,key,"molten");
		}
		else if (obj.state==="liquid") { // Evaporating
			if (!newname) {
				newname = key;
			...
			autoGen(newname,key,"vaporize");
		}
	}
	if (obj.tempLow!==undefined && (obj.stateLow===undefined||obj.forceAutoGen)) {
		var newname = obj.stateLowName;
		if (obj.state==="liquid" || !obj.state) { // Freezing
			if (!newname) {
				newname = key;
				if (newname.startsWith("liquid_")) { newname = newname.substring(7); }
			...
			autoGen(newname,key,"frozen");
		}
		else if (obj.state==="gas") { // Condensing
			if (!newname) {
				newname = key;
				if (newname.endsWith("_gas")) { newname = newname.substring(0,newname.length-4); }
				newname = "liquid_"+newname;
			}
			autoGen(newname,key,"condense");
		}
	}
	if (obj.behavior && typeof obj.behavior[0] === "string") {
		var newbehavior = [];
		for (var i = 0; i < obj.behavior.length; i++) {
			newbehavior.push(obj.behavior[i].split("|"));
		}
		obj.behavior = newbehavior;
	}
	if (obj.behaviorOn && typeof obj.behaviorOn[0] === "string") {
		var newbehavior = [];
		for (var i = 0; i < obj.behaviorOn.length; i++) {
			newbehavior.push(obj.behaviorOn[i].split("|"));
		}
		obj.behaviorOn = newbehavior;
	}
}
function finalizeColor(obj) {
	// if the element has no color, skip it
	if (obj.color === undefined || obj.colorObject !== undefined) {
		return;
	}
	...
	if (obj.color instanceof Array) {
		...
		obj.color = rgbs;
		obj.colorObject = rgbos;
	} else {
		// if obj.color starts with #
		if (obj.color.startsWith("#")) {
			var rgb = hexToRGB(obj.color);
			obj.color = "rgb("+rgb.r+","+rgb.g+","+rgb.b+")";
			obj.colorObject = rgb;
		}
	}
}
function finalizeElementAfter(key) {
...
function finalizeElement(key,obj) {
	finalizeColor(obj);
	checkAutoGen(key,obj);
	finalizeElementAfter(key);
}
function addElement(key,obj) {
	if (elements[key] !== undefined) {
		if (elements[key].hidden && settings.unhide !== 1) {
			hiddenCount--;
		}
		elementCount--;
	}
	...
	finalizeElementAfter(key);
	elementCount++;
	if (onAddElementList.length) {
		for (let i = 0; i < onAddElementList.length; i++) {
			onAddElementList[i](key,obj);
		}
	}
	if (!obj.hidden || settings.unhide === 1) {
		let isCurrent = false
		let btn = document.getElementById("elementButton-"+key);
		if (btn) {
			isCurrent = btn.getAttribute("current") === "true";
			btn.remove();
		}
		createElementButton(key);
		if (isCurrent) {
			document.getElementById("elementButton-"+key).setAttribute("current","true");
		}
	}
	else {
		hiddenCount++;
	}
	document.getElementById("elementCountSpan").innerText = langKey("guitemplate.elementCount","$1 elements, with $2 hidden",{1:elementCount,2:hiddenCount});
}
...
window.onload = function() {
	...
	//	 checkAutoGen(element,elements[element])
	// }
	...
	resizeCanvas = function(newHeight,newWidth,newPixelSize,clear,noFix) {
	...
	randomChoices = Object.keys(elements).filter(function(e) {
		let info = elements[e];
		if (info.excludeRandom === true) return false;
		if (info.canPlace === true) return true;
		return info.category !== "tools" && info.category !== "edit" && !info.tool;
	});
	gameCanvas.addEventListener("mousedown", function(e){
		if (!showingMenu && elements[currentElement] && elements[currentElement].onMouseDown && e.button == 0) {
		...
		if (url) {
			...
			if (file.type.indexOf('image/') !== -1) {
				...
				let fn = function() {
					var reader = new FileReader();
					reader.onload = function(e) {
						lastSaveJSON = e.target.result;
						loadSave(JSON.parse(e.target.result));
					}
					reader.readAsText(file);
					currentSaveData.filename = file.name;
				}
				if (currentPixels.length === 0 || settings.resetwarning === 0) fn();
				else {
					promptConfirm("Clear this scene and load save file?", (r) => { if (r) fn(); }, "Clear Canvas" );
				}
			}
		}
	}, false);
	...
	window.onbeforeunload = function(){
		if (currentPixels.length > 0 && !standalone && !forceReload){
	...
	document.addEventListener("keydown", function(e) {
		...
		if (e.ctrlKey || e.metaKey) {
			if (e.keyCode == 83) { // Ctrl+S = Save
				...
				e.preventDefault();
				if (e.shiftKey) {
					if (lastSaveJSON !== null) loadSave(JSON.parse(lastSaveJSON));
				}
				else loadFromFile();
			}
			return
		...
		if (showingMenu) {
			...
			else if (e.keyCode == 13 && showingMenu == "prompt" && promptState && promptState.type !== "input" && (e.target.tagName !== "INPUT")) {
				handlePrompt(true);
			}
			// l to closeMenu() if showing saves
			else if (e.keyCode == 76 && showingMenu == "saves" && e.target.tagName !== "INPUT") {
				e.preventDefault();
				closeMenu();
			}
			return;
	...
	startingWindowHeight = window.innerHeight;
	// notice when window is resized
	window.addEventListener("resize", function(e) {
		if (Math.abs(startingWindowWidth - window.innerWidth) >= 300) {
			startingWindowWidth = window.innerWidth;
			startingWindowHeight = window.innerHeight;
			if (currentPixels.length === 0) {
				clearAll();
			}
			else if (!settings.resizeNotice) {
				promptText("Reset the canvas by pressing 'R' or the 'Reset' button.",()=>{ saveSettings(); },"Window Resized")
				settings.resizeNotice = true;
			}
		}
	});
	...
	document.getElementById("extraInfo").insertAdjacentHTML("beforeend", "<small class='extraInfo-right'><p><span id='extraInfo-name'>Sandboxels </span>v" + currentversion + " • <span id='elementCountSpan'>" +
		langKey("guitemplate.elementCount","$1 elements, with $2 hidden",{1:elementCount,2:hiddenCount}) +
		"</span>.</p></small><small id='attribution'><p>©2021-" + new Date().getFullYear() + ". <a href='https://sandboxels.R74n.com/license.txt' rel='license' target='_blank' id='copyrightLabel'>All Rights Reserved</a>. <a style='color:#00ffff' rel='author' href='https://r74n.com'>R74n</a></p></small>");
	...
	if (location.ancestorOrigins && location.ancestorOrigins[0] && standaloneType !== "discord" && location.hostname !== "neal.fun") {
		//lol
	...
	var firstDiv = document.querySelector("#elementControls .category");
	...
	if (tempSave) {
		...
		if (saveID) loadSaveURL("https://sandboxels.live/browser/api/save/" + urlParams.get("discord"));
	}
	let tempMods = localStorage.getItem("SandboxelsTempMods");
	if (tempMods) {
		JSON.parse(tempMods).forEach((mod) => {
			removeMod(mod,true);
		})
		localStorage.removeItem("SandboxelsTempMods")
	}
...
(function bootstrap() {
  ...
  const text = (s, x, y, col, a, scale) => {
    ...
    nx.gfx.text("" + s, x | 0, y | 0, c[0], c[1], c[2], a == null ? 255 : a, scale || 2);
  };
  const rect = (x, y, w, h, col, a) => {
    const c = G.__sbxParseColor(col || "#000000");
    nx.gfx.fillRect(x | 0, y | 0, w | 0, h | 0, c[0], c[1], c[2], a == null ? 255 : a);
  };
  const title = (name) => {
    const t = "" + name;
  ...
  try {
    ...
    if (typeof canvasLayers === "object" && canvasLayers) {
      for (const k of Object.keys(canvasLayers)) {
        ...
        if (c && c instanceof G.__sbxHTMLCanvas) { c._primary = true; c._buf = null; }
      }
    }

    paused = false;
    if (typeof manualPaused !== "undefined") manualPaused = false;

    function refreshBg() {
      const c = G.__sbxParseColor((typeof settings === "object" && settings.bg) || "#000000");
      G.__sbxBg = [c[0], c[1], c[2]];
    }
    refreshBg();
    ...
    function buildCatalogue() {
      ...
      for (const c of order) map[c].sort();
      return { order, map };
    }
    ...
    const COLS = 6, CELL_W = Math.floor((SCREEN_W - 16) / COLS), CELL_H = 56;
    const GRID_X = 8;
    const VIS_ROWS = Math.floor((SCREEN_H - GRID_TOP - 8) / CELL_H);
    ...
    function pickerHitTest(tx, ty) {
      if (ty < tabRows * TAB_H) {
        ...
        if (col < TABS_PER_ROW && i < cat.order.length) return { type: "tab", i };
        return null;
      }
      if (ty < GRID_TOP) return null;
      const col = Math.floor((tx - GRID_X) / CELL_W);
      const row = Math.floor((ty - GRID_TOP) / CELL_H);
      if (col < 0 || col >= COLS || row < 0 || row >= VIS_ROWS) return null;
      const idx = page * PER_PAGE + row * COLS + col;
      const list = curList();
      if (idx >= 0 && idx < list.length) return { type: "cell", name: list[idx] };
      return null;
    }
    ...
    function handleInput() {
      ...
      if (hit & B.ZR) mouseSize = Math.min(50, mouseSize + 2);
      if (hit & B.ZL) mouseSize = Math.max(1, mouseSize - 2);
      if ((hit & B.Minus) && typeof setView === "function") {
        try { setView((view % viewCount) + 1); } catch (e) {}
      }
      ...
      if (drawing) {
        lastPos = wasDrawing ? { x: lastDrawPos.x, y: lastDrawPos.y } : { x: gx, y: gy };
        lastDrawPos = { x: gx, y: gy };
        mouseType = placing ? "left" : "right";
        mouseIsDown = true;
      } else {
        mouseIsDown = false;
      }
    ...
    function drawHud() {
      ...
      rect(6, 4, 22, 22, elColor(currentElement), 255);
      text(title(currentElement), 34, 8, "#ffffff", 255, 2);
      ...
      const sz = mouseSize * pixelSize;
      rect(cx - half, cy - half, sz, 1, "#ffffff", 200);
    ...
    for (;;) {
      ...
      if (!pickerOpen) {
        try { G.__sbxRunTimers(G.__sbxNow()); }
        catch (e) { log("tick err: " + e + "\n" + (e && e.stack)); }
      }
      render();
      const elapsed = G.__sbxNow() - frameStart;
      if (elapsed < TARGET_FRAME_MS) nx.sleep(TARGET_FRAME_MS - elapsed);
    }
  } catch (e) {
    log("FATAL: " + e + "\n" + (e && e.stack));
  } finally {
