...
#if !defined(XP_WIN) && !defined(__wasi__)
  // newlib/libnx pthread_cond_t size is not a multiple of sizeof(void*); use
  // byte storage sized to it with pointer alignment (see PlatformMutex.h).
  alignas(void*) unsigned char platformData_[sizeof(pthread_cond_t)];
#else
