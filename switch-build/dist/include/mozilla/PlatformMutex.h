...
#if !defined(XP_WIN) && !defined(__wasi__)
  // newlib/libnx pthread_mutex_t (int type + lock union) is not a multiple of
  // sizeof(void*); use byte storage sized to it with pointer alignment instead
  // of requiring an exact pointer-multiple size.
  alignas(void*) unsigned char platformData_[sizeof(pthread_mutex_t)];
#else
